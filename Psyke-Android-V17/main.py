from __future__ import annotations

import os
import sys
import time

# Use ANGLE (DirectX backend) when no proper OpenGL 2.0+ driver is available
# (e.g. RDP sessions, VMs, missing GPU drivers — "GDI Generic" renderer)
if sys.platform == "win32":
    os.environ.setdefault("KIVY_GL_BACKEND", "glew")
    from gl_preflight import ensure_compatible_opengl
    ensure_compatible_opengl()

from datetime import datetime

from kivy.animation import Animation
from kivy.app import App
from kivy.clock import Clock
from kivy.core.text import Label as CoreLabel, LabelBase
from kivy.core.window import Window
from kivy.graphics import (
    Color, Ellipse, Line, Rectangle, RoundedRectangle,
    ScissorPush, ScissorPop, PushMatrix, PopMatrix, Rotate,
)
from kivy.metrics import dp, sp
from kivy.properties import BooleanProperty, NumericProperty
from kivy.uix.boxlayout import BoxLayout
from kivy.uix.floatlayout import FloatLayout
from kivy.uix.screenmanager import ScreenManager, Screen, NoTransition
from kivy.uix.scrollview import ScrollView
from kivy.uix.widget import Widget

from kivymd.app import MDApp
from kivymd.uix.boxlayout import MDBoxLayout
from kivymd.uix.button import MDFlatButton, MDRaisedButton
from kivymd.uix.card import MDCard
from kivymd.uix.dialog import MDDialog
from kivymd.uix.label import MDLabel
from kivymd.uix.snackbar import MDSnackbar
from kivymd.uix.textfield import MDTextField

from models import (
    SanityState, FearManager, UndoStack, SaveManager,
    SANITY_BASE, clamp,
    WIS_MIN, WIS_MAX, CON_MIN, CON_MAX
)
from widgets import SanityBar, MadnessBanner, ExhaustionWidget
from tab_fears import FearsTab
from tab_sanity import SanityTab
from tab_wounds import WoundsTab
from tab_spells import SpellsTab
from ui_utils import NotificationActionButton
import theme as T

# ── Unicode font — DejaVuSans ships with Kivy and covers arrows, symbols, etc.
_dv_path = os.path.join(__import__('kivy').kivy_data_dir, 'fonts', 'DejaVuSans.ttf')
if os.path.exists(_dv_path):
    LabelBase.register(name='Symbols', fn_regular=_dv_path)

try:
    import build_info as BI
except Exception:
    class _BI:
        APP_VERSION = "unknown"
        BUILD_SHA = "unknown"
    BI = _BI()


# ═══════════════════════════════════════════════════════════════════════════
# SESSION LOG WIDGET
# ═══════════════════════════════════════════════════════════════════════════

class SessionLog(MDBoxLayout):
    """Scrollable session log panel used inside a dialog."""

    def __init__(self, **kwargs):
        super().__init__(orientation="vertical", spacing=dp(4),
                         padding=dp(8), **kwargs)
        self._entries: list[str] = []

        hdr = MDBoxLayout(size_hint_y=None, height=dp(36), spacing=dp(8))
        hdr.add_widget(MDLabel(
            text="SESSION LOG", bold=True,
            theme_text_color="Custom", text_color=T.k(T.BLUE),
            font_style="Button"))
        copy_btn = MDFlatButton(
            text="Copy All", size_hint_x=None, width=dp(80),
            theme_text_color="Custom", text_color=T.k(T.TEXT_DIM),
            on_release=self._copy_all)
        hdr.add_widget(Widget())
        hdr.add_widget(copy_btn)
        self.add_widget(hdr)

        self._log_sv = ScrollView(size_hint=(1, 1), do_scroll_x=False)
        self._log_box = MDBoxLayout(
            orientation="vertical", spacing=dp(2),
            size_hint_y=None, adaptive_height=True, padding=dp(4))
        self._log_sv.add_widget(self._log_box)
        self.add_widget(self._log_sv)

    def add_entry(self, msg: str):
        ts   = datetime.now().strftime("%H:%M")
        full = f"[{ts}] {msg}"
        self._entries.append(full)
        lbl = MDLabel(
            text=full,
            theme_text_color="Custom",
            text_color=self._color_for(msg),
            font_style="Caption",
            size_hint_y=None, adaptive_height=True)
        self._log_box.add_widget(lbl)
        Clock.schedule_once(lambda dt: setattr(self._log_sv, "scroll_y", 0))

    def _color_for(self, msg: str):
        m = msg.upper()
        if "ENCOUNTER" in m: return T.k(T.GOLD)
        if "WOUND" in m:     return T.k(T.BLOOD)
        if "MADNESS" in m or "THRESHOLD" in m: return T.k(T.PURPLE)
        if "RESTORATION" in m: return T.k(T.SILVER)
        if "PASS" in m:      return T.k(T.GREEN)
        if "FAIL" in m:      return T.k(T.RED)
        if "WARNING" in m:   return T.k(T.BLOOD_LT)
        return T.k(T.TEXT_DIM)

    def _copy_all(self, *_):
        from kivy.core.clipboard import Clipboard
        Clipboard.copy("\n".join(self._entries))
        MDSnackbar(
            MDLabel(text="Log copied to clipboard.",
                    theme_text_color="Custom", text_color=(1, 1, 1, 1)),
            md_bg_color=T.k(T.BG_CARD), duration=2
        ).open()

    def clear(self):
        self._entries.clear()
        self._log_box.clear_widgets()


# ═══════════════════════════════════════════════════════════════════════════
# STAT DIALOG
# ═══════════════════════════════════════════════════════════════════════════

class StatDialog:
    """Reusable dialog for editing WIS or CON score."""

    def __init__(self, title: str, current: int, on_confirm):
        self._field = MDTextField(
            hint_text="Score (1–30)",
            text=str(current),
            input_filter="int",
            mode="rectangle",
            line_color_normal=T.k(T.BORDER),
            line_color_focus=T.k(T.GOLD_LT),
            hint_text_color_focus=T.k(T.GOLD_LT),
        )
        self._field.text_color_focus = T.k(T.TEXT_BRIGHT)
        self._field.foreground_color = T.k(T.TEXT_BRIGHT)
        self._dlg = MDDialog(
            title=title,
            type="custom",
            content_cls=self._field,
            buttons=[
                MDFlatButton(text="Cancel", on_release=self._close),
                MDRaisedButton(
                    text="Set",
                    md_bg_color=T.k(T.PURPLE),
                    on_release=lambda *_: self._confirm(on_confirm)),
            ]
        )

    def open(self): self._dlg.open()
    def _close(self, *_): self._dlg.dismiss()

    def _confirm(self, cb):
        try:
            val = int(clamp(int(self._field.text.strip()), 1, 30))
            cb(val)
        except Exception:
            pass
        self._dlg.dismiss()


# ═══════════════════════════════════════════════════════════════════════════
# HEADER CARD
# ═══════════════════════════════════════════════════════════════════════════

class _AdvBtn(BoxLayout):
    """Tappable ADV toggle — sits on canvas above stat chips."""

    def __init__(self, color_hex: str, on_toggle, **kwargs):
        kwargs.setdefault("size_hint_x", None)
        kwargs.setdefault("width", dp(76))
        super().__init__(**kwargs)
        self._active    = False
        self._color     = color_hex
        self._on_toggle = on_toggle
        self._lbl = MDLabel(
            text="ADV",
            font_style="Caption", bold=False,
            theme_text_color="Custom", text_color=T.k(T.TEXT_DIM, 0.45),
            halign="center")
        self.add_widget(self._lbl)

    def set_scale(self, scale: float):
        self._lbl.font_size = f"{max(9.0, 11.0 * scale):.1f}sp"

    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            self.set_active(not self._active)
            self._on_toggle(self._active)
            return True
        return super().on_touch_down(touch)

    def set_active(self, active: bool):
        self._active = bool(active)
        self._lbl.bold = self._active
        self._lbl.text_color = (
            T.k(self._color) if self._active else T.k(T.TEXT_DIM, 0.45)
        )


class _StatChip(BoxLayout):
    """
    Vertical stat badge for WIS / CON.

    Two-section layout:
      Top  — stat name (Caption) + large score number
      Bot  — modifier centred in a tinted panel

    Tap anywhere to open the edit dialog.
    """

    def __init__(self, label: str, color_hex: str, on_tap, **kwargs):
        kwargs.setdefault("orientation", "vertical")
        kwargs.setdefault("size_hint_x", None)
        kwargs.setdefault("width", dp(92))
        kwargs.setdefault("padding", [dp(4), dp(4), dp(4), dp(4)])
        kwargs.setdefault("spacing", dp(2))
        super().__init__(**kwargs)
        self._color  = color_hex
        self._on_tap = on_tap
        self._bot_h  = dp(20)

        with self.canvas.before:
            Color(*T.k(color_hex, 0.6))
            self._bd = Rectangle()
            Color(*T.k(T.BG_INSET))
            self._bg = Rectangle()
            Color(*T.k(color_hex, 0.13))
            self._bot_bg = Rectangle()
            Color(*T.k(color_hex, 0.5))
            self._sep = Rectangle()
        self.bind(pos=self._upd, size=self._upd)

        # ── Top section ───────────────────────────────────────────────────────
        top = MDBoxLayout(orientation="vertical", spacing=dp(0))

        self._label_lbl = MDLabel(
            text=label,
            font_style="Caption", bold=True,
            theme_text_color="Custom", text_color=T.k(color_hex),
            halign="center", size_hint_y=None, height=dp(14))

        self._score_lbl = MDLabel(
            text="10",
            bold=True, font_size="22sp",
            theme_text_color="Custom", text_color=T.k(color_hex),
            halign="center", size_hint_y=None, height=dp(28))

        top.add_widget(self._label_lbl)
        top.add_widget(self._score_lbl)

        # ── Bottom section — modifier centred ─────────────────────────────────
        bot = MDBoxLayout(
            size_hint_y=None, height=self._bot_h,
            padding=[dp(4), dp(0), dp(4), dp(0)])

        self._mod_lbl = MDLabel(
            text="+0",
            bold=True, font_size="15sp",
            theme_text_color="Custom", text_color=T.k(color_hex),
            halign="center")

        bot.add_widget(self._mod_lbl)

        self._top = top
        self._bot = bot
        self.add_widget(top)
        self.add_widget(bot)

    def set_scale(self, scale: float):
        self.padding = [dp(4) * scale, dp(4) * scale, dp(4) * scale, dp(4) * scale]
        self.spacing = dp(2) * scale
        self._bot_h = dp(20) * scale
        self._bot.height = self._bot_h
        self._bot.padding = [dp(4) * scale, 0, dp(4) * scale, 0]
        self._label_lbl.height = dp(14) * scale
        self._label_lbl.font_size = f"{max(8.0, 11.0 * scale):.1f}sp"
        self._score_lbl.height = dp(28) * scale
        self._score_lbl.font_size = f"{max(16.0, 22.0 * scale):.1f}sp"
        self._mod_lbl.font_size = f"{max(11.0, 15.0 * scale):.1f}sp"
        self._upd()

    def _upd(self, *_):
        self._bd.pos  = self.pos
        self._bd.size = self.size
        self._bg.pos  = (self.x + 1, self.y + 1)
        self._bg.size = (max(0, self.width - 2), max(0, self.height - 2))
        tint_h = dp(4) + self._bot_h
        self._bot_bg.pos  = (self.x + 1, self.y + 1)
        self._bot_bg.size = (max(0, self.width - 2), max(0, tint_h - 1))
        self._sep.pos  = (self.x + 1, self.y + tint_h)
        self._sep.size = (max(0, self.width - 2), dp(1))

    def update(self, score: int, mod: int):
        self._score_lbl.text = str(score)
        self._mod_lbl.text   = f"{mod:+d}"

    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            self._on_tap()
            return True
        return super().on_touch_down(touch)


class _SanityChip(BoxLayout):
    """Sanity chip: base+WIS=MAX equation, current/max value, and percentage."""

    def __init__(self, **kwargs):
        kwargs.setdefault("orientation", "vertical")
        kwargs.setdefault("size_hint_x", None)
        kwargs.setdefault("width", dp(92))
        kwargs.setdefault("spacing", dp(2))
        kwargs.setdefault("padding", [dp(2), dp(2), dp(2), dp(2)])
        super().__init__(**kwargs)

        with self.canvas.before:
            Color(*T.k(T.PURPLE, 0.6))
            self._bd = Rectangle()
            Color(*T.k(T.BG_INSET))
            self._bg = Rectangle()
        self.bind(pos=self._upd, size=self._upd)

        # Title row — "SANITY" label matching _StatChip style
        self._title_lbl = MDLabel(
            text="SANITY",
            font_style="Caption", bold=True,
            theme_text_color="Custom", text_color=T.k(T.PURPLE),
            halign="center",
            size_hint_y=None, height=dp(14))
        # Formula row: SANITY_BASE + WIS = MAX
        self._eq_lbl = MDLabel(
            text=f"{SANITY_BASE}+10=20",
            font_style="Caption",
            theme_text_color="Custom", text_color=T.k(T.TEXT_DIM),
            halign="center",
            size_hint_y=None, height=dp(12))
        # Current / Max
        self._value_lbl = MDLabel(
            text="20/20",
            bold=True, font_size="18sp",
            theme_text_color="Custom", text_color=T.k(T.PURPLE_LT),
            halign="center",
            size_hint_y=None, height=dp(22))
        # Percentage in purple
        self._pct_lbl = MDLabel(
            text="100%",
            bold=True, font_size="13sp",
            theme_text_color="Custom", text_color=T.k(T.PURPLE),
            halign="center",
            size_hint_y=None, height=dp(12))
        self.add_widget(self._title_lbl)
        self.add_widget(self._eq_lbl)
        self.add_widget(self._value_lbl)
        self.add_widget(self._pct_lbl)

    def set_scale(self, scale: float):
        self.padding = [dp(2) * scale, dp(2) * scale, dp(2) * scale, dp(2) * scale]
        self.spacing = dp(2) * scale
        self._title_lbl.height = dp(14) * scale
        self._title_lbl.font_size = f"{max(8.0, 11.0 * scale):.1f}sp"
        self._eq_lbl.height = dp(12) * scale
        self._eq_lbl.font_size = f"{max(7.0, 9.0 * scale):.1f}sp"
        self._value_lbl.height = dp(22) * scale
        self._value_lbl.font_size = f"{max(13.0, 18.0 * scale):.1f}sp"
        self._pct_lbl.height = dp(12) * scale
        self._pct_lbl.font_size = f"{max(10.0, 13.0 * scale):.1f}sp"
        self._upd()

    def _upd(self, *_):
        self._bd.pos  = self.pos
        self._bd.size = self.size
        self._bg.pos  = (self.x + 1, self.y + 1)
        self._bg.size = (max(0, self.width - 2), max(0, self.height - 2))

    def update(self, current: int, max_san: int, pct: float,
               _madness_stage=None, wis_score: int = None):
        if wis_score is not None:
            self._eq_lbl.text = f"{SANITY_BASE}+{wis_score}={max_san}"
        self._value_lbl.text = f"{current}/{max_san}"
        self._pct_lbl.text   = f"{pct:.0f}%"


class _HopePortrait(BoxLayout):
    """
    Campfire hope toggle — no image picker, campfire icon only.
    Unlit campfire = no hope; lit campfire = hope active.
    Tap to toggle.
    """

    fire_t = NumericProperty(0.0)

    def __init__(self, on_toggle, on_image_picked=None, **kwargs):  # noqa: on_image_picked unused by design
        kwargs.setdefault("orientation", "vertical")
        kwargs.setdefault("size_hint_x", None)
        kwargs.setdefault("width", dp(56))
        kwargs.setdefault("spacing", dp(2))
        super().__init__(**kwargs)
        self._active    = False
        self._on_toggle = on_toggle

        self._port = Widget(size_hint_y=1)
        self._port.bind(pos=self._redraw, size=self._redraw)
        self.bind(fire_t=self._redraw)
        Clock.schedule_once(self._redraw)

        self._lbl = MDLabel(
            text="HOPE", font_style="Caption", bold=False,
            theme_text_color="Custom", text_color=T.k(T.TEXT_DIM),
            halign="center", size_hint_y=None, height=dp(14))

        self.add_widget(self._port)
        self.add_widget(self._lbl)

    def set_scale(self, scale: float):
        self.spacing = dp(2) * scale
        self._lbl.height = dp(14) * scale
        self._lbl.font_size = f"{max(8.0, 11.0 * scale):.1f}sp"
        self._redraw()

    @staticmethod
    def _mix_rgba(from_hex: str, to_hex: str, t: float,
                  from_alpha: float = 1.0, to_alpha: float = 1.0):
        start = T.k(from_hex, from_alpha)
        end = T.k(to_hex, to_alpha)
        return tuple(start[i] + (end[i] - start[i]) * t for i in range(4))

    def _animate_fire(self):
        target = 1.0 if self._active else 0.0
        Animation.cancel_all(self, "fire_t")
        if abs(self.fire_t - target) < 0.001:
            self.fire_t = target
            return
        duration = 0.22 if target > self.fire_t else 0.18
        Animation(fire_t=target, duration=duration, t="out_cubic").start(self)

    def _redraw(self, *_):
        w = self._port
        d  = max(min(w.width, w.height) - dp(2), dp(10))
        bw = dp(3) if self._active else dp(2)
        x  = w.x + (w.width  - d) / 2
        y  = w.y + (w.height - d) / 2
        c  = self._accent()
        w.canvas.clear()
        with w.canvas:
            # Outer border ring
            Color(*T.k(c))
            Ellipse(pos=(x, y), size=(d, d))
            # Inner dark fill
            id_ = d - bw * 2
            ix, iy = x + bw, y + bw
            Color(*T.k(T.BG_INSET))
            Ellipse(pos=(ix, iy), size=(id_, id_))
            # Campfire icon
            cx = ix + id_ / 2
            if self._active:
                # ── Lit campfire ──────────────────────────────────────────
                # Stone ring drop-shadow
                Color(0.0, 0.0, 0.0, 0.55)
                Ellipse(pos=(cx - id_*0.29, iy + id_*0.03),
                        size=(id_*0.58, id_*0.14))
                # Stone ring
                Color(*T.k(T.BORDER_LT, 0.92))
                Ellipse(pos=(cx - id_*0.27, iy + id_*0.06),
                        size=(id_*0.54, id_*0.13))
                # Hot ember bed inside ring
                Color(0.72, 0.24, 0.04, 0.65)
                Ellipse(pos=(cx - id_*0.18, iy + id_*0.08),
                        size=(id_*0.36, id_*0.07))
                # Logs (behind flame base)
                Color(0.36, 0.18, 0.06, 1.0)
                Line(points=[ix + id_*0.18, iy + id_*0.14,
                             ix + id_*0.70, iy + id_*0.46], width=dp(2.5))
                Line(points=[ix + id_*0.82, iy + id_*0.14,
                             ix + id_*0.30, iy + id_*0.46], width=dp(2.5))
                # Outer heat bloom (diffuse red-orange halo)
                Color(0.78, 0.20, 0.04, 0.20)
                Ellipse(pos=(cx - id_*0.30, iy + id_*0.20),
                        size=(id_*0.60, id_*0.66))
                # Main flame — orange body
                Color(0.87, 0.42, 0.07, 0.90)
                Ellipse(pos=(cx - id_*0.19, iy + id_*0.24),
                        size=(id_*0.38, id_*0.55))
                # Mid flame — gold-orange
                Color(*T.k(T.GOLD, 0.96))
                Ellipse(pos=(cx - id_*0.12, iy + id_*0.34),
                        size=(id_*0.24, id_*0.40))
                # Inner flame — bright gold
                Color(*T.k(T.GOLD_LT, 1.0))
                Ellipse(pos=(cx - id_*0.07, iy + id_*0.46),
                        size=(id_*0.14, id_*0.24))
                # Hot tip — near white
                Color(1.0, 0.97, 0.76, 0.88)
                Ellipse(pos=(cx - id_*0.025, iy + id_*0.61),
                        size=(id_*0.05, id_*0.08))
            else:
                # ── Unlit campfire ────────────────────────────────────────
                # Stone ring drop-shadow
                Color(0.0, 0.0, 0.0, 0.40)
                Ellipse(pos=(cx - id_*0.29, iy + id_*0.03),
                        size=(id_*0.58, id_*0.14))
                # Stone ring
                Color(*T.k(T.BORDER, 0.68))
                Ellipse(pos=(cx - id_*0.27, iy + id_*0.06),
                        size=(id_*0.54, id_*0.13))
                # Cold ash inside ring
                Color(0.24, 0.26, 0.28, 0.55)
                Ellipse(pos=(cx - id_*0.18, iy + id_*0.08),
                        size=(id_*0.36, id_*0.07))
                # Logs (cold, grey-brown)
                Color(*T.k(T.TEXT_DIM, 0.72))
                Line(points=[ix + id_*0.18, iy + id_*0.14,
                             ix + id_*0.70, iy + id_*0.46], width=dp(2.5))
                Line(points=[ix + id_*0.82, iy + id_*0.14,
                             ix + id_*0.30, iy + id_*0.46], width=dp(2.5))
                # Smoke wisps (thin bezier curves)
                Color(0.42, 0.44, 0.46, 0.30)
                Line(bezier=[cx, iy + id_*0.50,
                             cx + id_*0.06, iy + id_*0.60,
                             cx - id_*0.04, iy + id_*0.70,
                             cx + id_*0.02, iy + id_*0.80],
                     width=dp(1.0))
                Color(0.42, 0.44, 0.46, 0.18)
                Line(bezier=[cx - id_*0.05, iy + id_*0.52,
                             cx - id_*0.10, iy + id_*0.63,
                             cx - id_*0.03, iy + id_*0.73,
                             cx - id_*0.08, iy + id_*0.82],
                     width=dp(1.0))

    def _redraw(self, *_):
        w = self._port
        d = max(min(w.width, w.height) - dp(2), dp(10))
        bw = dp(2) + dp(1) * self.fire_t
        x = w.x + (w.width - d) / 2
        y = w.y + (w.height - d) / 2

        outer_rgba = self._mix_rgba(T.BORDER_LT, T.GOLD_LT, self.fire_t)
        label_rgba = self._mix_rgba(T.TEXT_DIM, T.GOLD_LT, self.fire_t)
        self._lbl.text_color = label_rgba
        self._lbl.bold = self._active or self.fire_t >= 0.55

        w.canvas.clear()
        with w.canvas:
            Color(*outer_rgba)
            Ellipse(pos=(x, y), size=(d, d))

            inner_d = d - bw * 2
            ix, iy = x + bw, y + bw
            Color(*T.k(T.BG_INSET))
            Ellipse(pos=(ix, iy), size=(inner_d, inner_d))

            fire_level = self.fire_t
            flame_alpha = 0.08 + fire_level * 0.88
            heat_alpha = max(0.0, (fire_level - 0.18) / 0.82) * 0.22
            spark_alpha = max(0.0, (fire_level - 0.40) / 0.60) * 0.95
            ember_alpha = 0.24 + (1.0 - fire_level) * 0.34
            ring_alpha = 0.70 + fire_level * 0.20
            log_alpha = 0.72 + fire_level * 0.20

            cx = ix + inner_d / 2
            fire_pad = inner_d * 0.14
            fire_w = inner_d - fire_pad * 2
            fire_h = inner_d - fire_pad * 2
            fx = ix + fire_pad
            fy = iy + fire_pad * 0.90

            Color(0.0, 0.0, 0.0, 0.30 + fire_level * 0.20)
            Ellipse(pos=(cx - fire_w * 0.28, fy + fire_h * 0.08),
                    size=(fire_w * 0.56, fire_h * 0.12))
            Color(*T.k(T.BORDER_LT, ring_alpha))
            Ellipse(pos=(cx - fire_w * 0.27, fy + fire_h * 0.11),
                    size=(fire_w * 0.54, fire_h * 0.11))
            Color(0.72, 0.24, 0.04, ember_alpha)
            Ellipse(pos=(cx - fire_w * 0.18, fy + fire_h * 0.14),
                    size=(fire_w * 0.36, fire_h * 0.06))
            Color(0.36, 0.18, 0.06, log_alpha)
            Line(points=[fx + fire_w * 0.20, fy + fire_h * 0.20,
                         fx + fire_w * 0.68, fy + fire_h * 0.46],
                 width=dp(2.2))
            Line(points=[fx + fire_w * 0.80, fy + fire_h * 0.20,
                         fx + fire_w * 0.32, fy + fire_h * 0.46],
                 width=dp(2.2))
            Color(0.78, 0.20, 0.04, heat_alpha)
            Ellipse(pos=(cx - fire_w * 0.30, fy + fire_h * 0.24),
                    size=(fire_w * 0.60, fire_h * (0.26 + fire_level * 0.34)))

            flame_scale = 0.40 + fire_level * 0.60
            Color(0.87, 0.42, 0.07, flame_alpha * 0.90)
            Ellipse(pos=(cx - fire_w * 0.19 * flame_scale, fy + fire_h * 0.24),
                    size=(fire_w * 0.38 * flame_scale,
                          fire_h * 0.55 * flame_scale))
            Color(*T.k(T.GOLD, flame_alpha * 0.96))
            Ellipse(pos=(cx - fire_w * 0.12 * flame_scale, fy + fire_h * 0.34),
                    size=(fire_w * 0.24 * flame_scale,
                          fire_h * 0.40 * flame_scale))
            Color(*T.k(T.GOLD_LT, flame_alpha))
            Ellipse(pos=(cx - fire_w * 0.07 * flame_scale, fy + fire_h * 0.46),
                    size=(fire_w * 0.14 * flame_scale,
                          fire_h * 0.24 * flame_scale))
            Color(1.0, 0.97, 0.76, spark_alpha)
            Ellipse(pos=(cx - fire_w * 0.025,
                         fy + fire_h * (0.64 + fire_level * 0.08)),
                    size=(fire_w * 0.05, fire_h * 0.08))

    def set_state(self, active: bool, img_path: str = ""):  # noqa: img_path unused by design
        self._active = active
        self._redraw()
        self._animate_fire()

    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            self._active = not self._active
            self._redraw()
            self._animate_fire()
            self._on_toggle(self._active)
            return True
        return super().on_touch_down(touch)


class HeaderCard(BoxLayout):
    """
    Persistent header: character name, hope portrait, sanity chip,
    ADV toggles, WIS/CON stat chips, exhaustion pips.
    """

    _BASE_TOTAL_W = dp(396)

    def __init__(self, **kwargs):
        kwargs.setdefault("orientation", "vertical")
        kwargs.setdefault("size_hint_y", None)
        kwargs.setdefault("height", dp(182))
        kwargs.setdefault("spacing", dp(2))
        kwargs.setdefault("padding", [dp(8), dp(10), dp(8), dp(4)])
        super().__init__(**kwargs)

        with self.canvas.before:
            Color(*T.k(T.BG_CARD))
            self._bg = Rectangle()
        self.bind(pos=self._upd_bg, size=self._upd_bg)
        self._build()
        self.bind(width=self._apply_responsive_layout)
        Clock.schedule_once(self._apply_responsive_layout, 0)

    def _upd_bg(self, *_):
        self._bg.pos  = self.pos
        self._bg.size = self.size

    @staticmethod
    def _mix(a: float, b: float, t: float) -> float:
        return a + (b - a) * t

    def _build(self):
        # Row 1 — character name + exhaustion (72dp)
        row1 = MDBoxLayout(size_hint_y=None, height=dp(72), spacing=dp(6))
        self._name_field = MDTextField(
            hint_text="Character Name.",
            text="",
            mode="rectangle",
            line_color_normal=T.k(T.BORDER),
            line_color_focus=(1, 1, 1, 0.85),
            hint_text_color_focus=(1, 1, 1, 0.55),
            font_size="14sp")
        self._name_field.text_color_focus = T.k(T.TEXT_BRIGHT)
        self._name_field.foreground_color = T.k(T.TEXT_BRIGHT)
        self._name_field.bind(text=self._on_name_change)
        row1.add_widget(self._name_field)

        # Exhaustion: title + pips + description, right side of row1
        exh_side = BoxLayout(orientation="vertical", spacing=dp(2),
                             size_hint_x=None, width=dp(162))
        self._exh_widget = ExhaustionWidget(size_hint_y=None, height=dp(44))
        self._exh_widget.set_change_callback(self._on_exhaustion_change)

        exh_side.add_widget(self._exh_widget)
        row1.add_widget(exh_side)
        self.add_widget(row1)

        # ADV row — blank spacer (above reset) | CON ADV | WIS ADV | right spacer
        chip_w = dp(76)
        reset_w = dp(74)

        adv_row = MDBoxLayout(size_hint_y=None, height=dp(20), spacing=dp(6))
        adv_spacer = Widget(size_hint_x=None, width=reset_w)
        adv_row.add_widget(adv_spacer)
        self._con_adv = _AdvBtn(
            T.BLOOD,
            on_toggle=lambda v: setattr(App.get_running_app(), 'con_adv', v),
            size_hint_y=None, height=dp(20))
        self._wis_adv = _AdvBtn(
            T.PURPLE,
            on_toggle=lambda v: setattr(App.get_running_app(), 'wis_adv', v),
            size_hint_y=None, height=dp(20))
        adv_row.add_widget(self._con_adv)
        adv_row.add_widget(self._wis_adv)
        adv_row.add_widget(Widget())
        self.add_widget(adv_row)

        # Row 2 — Reset btn | CON chip | WIS chip | Sanity + Hope
        row2 = MDBoxLayout(size_hint_y=None, height=dp(72), spacing=dp(6))
        san_w = dp(84)
        hope_w = dp(54)

        self._reset_btn = _ResetBtn(
            on_tap=self._on_reset_request,
            width=reset_w,
            height=dp(72),
        )
        row2.add_widget(self._reset_btn)

        self._con_chip = _StatChip("CON", T.BLOOD, self._on_edit_con,
                                   size_hint_y=None, height=dp(72), width=chip_w)
        self._wis_chip = _StatChip("WIS", T.PURPLE, self._on_edit_wis,
                                   size_hint_y=None, height=dp(72), width=chip_w)
        row2.add_widget(self._con_chip)
        row2.add_widget(self._wis_chip)

        # Sanity chip then Hope campfire
        hope_san = MDBoxLayout(size_hint_x=None, width=san_w + dp(4) + hope_w, spacing=dp(4))
        self._san_chip = _SanityChip(size_hint_x=None, width=san_w, size_hint_y=1)
        self._hope_portrait = _HopePortrait(
            on_toggle=self._on_hope_toggle,
            on_image_picked=self._on_hope_image,
            size_hint_x=None, width=hope_w, size_hint_y=1)
        hope_san.add_widget(self._san_chip)
        hope_san.add_widget(self._hope_portrait)
        row2.add_widget(hope_san)
        self.add_widget(row2)

        self._row1 = row1
        self._row2 = row2
        self._adv_row = adv_row
        self._adv_spacer = adv_spacer
        self._exh_side = exh_side
        self._hope_san = hope_san

    def _apply_responsive_layout(self, *_):
        if self.width <= 0:
            return

        scale = clamp(self.width / self._BASE_TOTAL_W, 0.70, 1.08)
        compact_t = clamp((self._BASE_TOTAL_W - self.width) / dp(120), 0.0, 1.0)

        header_gap = round(self._mix(dp(2), dp(1), compact_t))
        row_gap = round(self._mix(dp(6), dp(3), compact_t))
        hope_gap = round(self._mix(dp(4), dp(2), compact_t))
        pad_x = round(self._mix(dp(8), dp(4), compact_t))
        pad_t = round(self._mix(dp(10), dp(6), compact_t))
        pad_b = round(self._mix(dp(4), dp(3), compact_t))

        row_h = max(round(dp(58)), round(dp(72) * scale))
        adv_h = max(round(dp(16)), round(dp(20) * scale))
        reset_w = max(round(dp(52)), round(dp(74) * scale))
        chip_w = max(round(dp(56)), round(dp(76) * scale))
        san_w = max(round(dp(62)), round(dp(84) * scale))
        hope_w = max(round(dp(40)), round(dp(54) * scale))
        exh_w = max(round(dp(128)), round(dp(162) * scale))
        exh_h = max(round(dp(36)), round(dp(44) * scale))

        self.padding = [pad_x, pad_t, pad_x, pad_b]
        self.spacing = header_gap
        self.height = row_h * 2 + adv_h + pad_t + pad_b + header_gap * 2

        self._row1.height = row_h
        self._row1.spacing = row_gap
        self._name_field.font_size = f"{max(11.0, 14.0 * scale):.1f}sp"
        self._exh_side.width = exh_w
        self._exh_side.spacing = max(1, round(dp(2) * scale))
        self._exh_widget.height = exh_h

        self._adv_row.height = adv_h
        self._adv_row.spacing = row_gap
        self._adv_spacer.width = reset_w
        self._con_adv.width = chip_w
        self._wis_adv.width = chip_w
        self._con_adv.set_scale(scale)
        self._wis_adv.set_scale(scale)

        self._row2.height = row_h
        self._row2.spacing = row_gap
        self._reset_btn.width = reset_w
        self._reset_btn.height = row_h
        self._reset_btn.set_scale(scale)

        self._con_chip.width = chip_w
        self._con_chip.height = row_h
        self._con_chip.set_scale(scale)
        self._wis_chip.width = chip_w
        self._wis_chip.height = row_h
        self._wis_chip.set_scale(scale)

        self._san_chip.width = san_w
        self._san_chip.set_scale(scale)
        self._hope_portrait.width = hope_w
        self._hope_portrait.set_scale(scale)
        self._hope_san.spacing = hope_gap
        self._hope_san.width = san_w + hope_gap + hope_w

    def refresh(self, state: SanityState):
        self._wis_chip.update(state.wis_score, state.wis_mod)
        self._con_chip.update(state.con_score, state.con_mod)
        self._exh_widget.level = state.exhaustion
        self._san_chip.update(state.current_sanity, state.max_sanity,
                              state.percent * 100, state.madness, state.wis_score)
        self._hope_portrait.set_state(state.hope,
                                      getattr(state, "hope_img_path", ""))
        app = App.get_running_app()
        self._wis_adv.set_active(getattr(app, "wis_adv", False))
        self._con_adv.set_active(getattr(app, "con_adv", False))

    def _on_hope_toggle(self, active: bool):
        app = App.get_running_app()
        app.state.hope = active
        app.save_manager.save(app.state, app.fm, app.char_name, app.enc_history)

    def _on_hope_image(self, path: str):
        app = App.get_running_app()
        app.state.hope_img_path = path
        app.save_manager.save(app.state, app.fm, app.char_name, app.enc_history)

    def _on_name_change(self, inst, val):
        App.get_running_app().char_name = val.strip()

    def _on_edit_wis(self, *_):
        app = App.get_running_app()
        def confirm(val):
            app.undo_stack.push(app.state, app.fm)
            app.state.wis_score   = val
            app.state.max_sanity  = SANITY_BASE + val
            app.state.current_sanity = min(app.state.current_sanity, app.state.max_sanity)
            app.state.rebuild_thresholds()
            app.refresh_all()
            app.save_manager.save(app.state, app.fm, app.char_name, app.enc_history)
        StatDialog("Edit WIS Score", app.state.wis_score, confirm).open()

    def _on_edit_con(self, *_):
        app = App.get_running_app()
        def confirm(val):
            app.undo_stack.push(app.state, app.fm)
            app.state.con_score = val
            app.refresh_all()
            app.save_manager.save(app.state, app.fm, app.char_name, app.enc_history)
        StatDialog("Edit CON Score", app.state.con_score, confirm).open()

    def _on_exhaustion_change(self, level: int):
        app = App.get_running_app()
        app.undo_stack.push(app.state, app.fm)
        app.state.exhaustion = level
        app.save_manager.save(app.state, app.fm, app.char_name, app.enc_history)

    def _on_reset_request(self, *_):
        App.get_running_app().confirm_reset()



# ═══════════════════════════════════════════════════════════════════════════
# TAB BUTTON
# ═══════════════════════════════════════════════════════════════════════════

class _TabBtn(BoxLayout):
    """Single tab button in the custom tab bar."""

    def __init__(self, label_text: str, active_color: str, on_tap, **kwargs):
        super().__init__(orientation="vertical", **kwargs)
        self._active_color = active_color
        self._on_tap = on_tap
        self._active = False

        self._lbl = MDLabel(
            text=label_text,
            halign="center",
            theme_text_color="Custom",
            text_color=T.k(T.TEXT_DIM),
            font_style="Button",
            bold=False)
        self.add_widget(self._lbl)
        self.bind(pos=self._draw, size=self._draw)
        Clock.schedule_once(self._draw)

    def set_active(self, active: bool):
        self._active = active
        self._lbl.text_color = T.k(self._active_color if active else T.TEXT_DIM)
        self._lbl.bold = active
        self._draw()

    def _draw(self, *_):
        self.canvas.before.clear()
        with self.canvas.before:
            if self._active:
                Color(*T.k(self._active_color, 0.14))
                Rectangle(pos=self.pos, size=self.size)
                Color(*T.k(self._active_color))
                Rectangle(pos=(self.x, self.y), size=(self.width, dp(3)))

    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            self._on_tap()
            return True
        return super().on_touch_down(touch)


class _ResetBtn(FloatLayout):
    """Compact hold-to-confirm reset button with right-side fill."""

    slide_t = NumericProperty(0.0)
    pressed = BooleanProperty(False)

    def __init__(self, on_tap, **kwargs):
        kwargs.setdefault("size_hint_x", None)
        kwargs.setdefault("size_hint_y", None)
        kwargs.setdefault("width", dp(68))
        kwargs.setdefault("height", dp(72))
        super().__init__(**kwargs)
        self._on_tap = on_tap
        self._triggered = False
        self._scale = 1.0
        self._slot_w = dp(17)
        self._radius = dp(5)
        self._text_layout_key = None
        self._reset_tex = None
        self._char_tex = None
        self._loop_tex = None
        self._loop_tex_key = None

        with self.canvas.before:
            self._border_col = Color(*T.k("#e62222"))
            self._border = RoundedRectangle(radius=[self._radius])
            self._base_bg_col = Color(*T.k(T.BG_INSET))
            self._base_bg = RoundedRectangle(radius=[max(0, self._radius - 1)])
            self._fill_col = Color(*T.k("#e62222"))
            self._fill = Rectangle()
            self._slot_col = Color(*T.k("#c41b1b"))
            self._slot_divider = Rectangle()

        self.bind(pos=self._redraw, size=self._redraw, slide_t=self._redraw, pressed=self._redraw)
        Clock.schedule_once(lambda *_: self._redraw(), 0)

    def set_scale(self, scale: float):
        self._scale = float(scale)
        self._slot_w = max(dp(12), dp(17) * self._scale)
        self._radius = max(dp(4), dp(5) * self._scale)
        self._text_layout_key = None
        self._redraw()

    def _fit_text_texture(self, text: str, max_w: float, max_h: float):
        if max_w <= 0 or max_h <= 0:
            return None
        tex = None
        base_fonts = (11.5, 11.0, 10.5, 10.0, 9.5, 9.0, 8.5, 8.0, 7.5)
        for font_sp in (fs * self._scale for fs in base_fonts):
            lbl = CoreLabel(
                text=text,
                bold=True,
                font_size=sp(font_sp),
                color=(1, 1, 1, 1),
            )
            lbl.refresh()
            tex = lbl.texture
            if tex and tex.width <= max_w and tex.height <= max_h:
                return tex
        return tex

    def _ensure_textures(self, max_w: float, max_h: float):
        key = (int(max_w), int(max_h))
        if key == self._text_layout_key:
            return
        self._text_layout_key = key
        self._reset_tex = self._fit_text_texture("Reset", max_w, max_h)
        self._char_tex = self._fit_text_texture("Character", max_w, max_h)

    def _ensure_loop_texture(self, font_sp: float):
        key = round(font_sp, 1)
        if key == self._loop_tex_key:
            return
        self._loop_tex_key = key
        lbl = CoreLabel(
            text="↻",
            bold=True,
            font_name="Symbols",
            font_size=sp(font_sp),
            color=(1, 1, 1, 1),
        )
        lbl.refresh()
        self._loop_tex = lbl.texture

    def _redraw(self, *_):
        inner_x = self.x + 1
        inner_y = self.y + 1
        inner_w = max(0, self.width - 2)
        inner_h = max(0, self.height - 2)
        fill_w = inner_w * self.slide_t

        # Border (always red outline)
        self._border.pos = self.pos
        self._border.size = self.size
        # Base bg = card colour — gives the "unfilled outline" look at rest
        self._base_bg.pos = (self.x + 1, self.y + 1)
        self._base_bg.size = (inner_w, inner_h)
        # Red fill sweeps in from the right as the hold progresses
        self._fill.pos = (inner_x + inner_w - fill_w, inner_y)
        self._fill.size = (fill_w, inner_h)

        # Slot divider on the right — fades out as fill covers it
        slot_alpha = max(0.0, 1.0 - self.slide_t * 1.35)
        self._slot_col.rgba = (*T.k("#c41b1b")[:3], slot_alpha)
        self._slot_divider.pos = (inner_x + inner_w - self._slot_w, inner_y + dp(4))
        self._slot_divider.size = (dp(1), max(0, inner_h - dp(8)))

        # Fit Reset / Character to the actual left panel so they never wrap or clip.
        left_x = inner_x + dp(1)
        left_w = max(0, inner_w - self._slot_w - dp(2))
        left_h = max(0, inner_h - dp(8))
        line_gap = 1
        row_h = max(0, (left_h - line_gap) / 2)
        text_shift = dp(18) * self._scale * self.slide_t
        label_opacity = max(0.0, 1.0 - self.slide_t * 1.8)
        self._ensure_textures(left_w, row_h)

        self.canvas.after.clear()

        if self._reset_tex and self._char_tex and label_opacity > 0.0:
            reset_x = round(left_x + max(0, (left_w - self._reset_tex.width) / 2))
            char_x = round(left_x + max(0, (left_w - self._char_tex.width) / 2))
            cluster_h = self._reset_tex.height + line_gap + self._char_tex.height
            cluster_y = round(inner_y + max(0, (inner_h - cluster_h) / 2))
            text_x_char = round(char_x - text_shift)
            text_x_reset = round(reset_x - text_shift)
            with self.canvas.after:
                ScissorPush(
                    x=int(inner_x),
                    y=int(inner_y),
                    width=max(0, int(inner_w)),
                    height=max(0, int(inner_h)),
                )
                Color(*T.k("#e62222", label_opacity))
                Rectangle(
                    texture=self._char_tex,
                    pos=(text_x_char, cluster_y),
                    size=self._char_tex.size,
                )
                Rectangle(
                    texture=self._reset_tex,
                    pos=(text_x_reset, cluster_y + self._char_tex.height + line_gap),
                    size=self._reset_tex.size,
                )
                ScissorPop()

        # Reset icon: starts in the right slot, grows and rotates into the centre.
        x_base_w = dp(15) * self._scale
        x_size_w = x_base_w + dp(35) * self._scale * self.slide_t
        x_font_sp = max(12.0, (13 + 16 * self.slide_t) * self._scale)
        x_slot_x = inner_x + inner_w - self._slot_w + (self._slot_w - x_base_w) / 2
        x_center_x = self.center_x - x_size_w / 2
        x_lbl_x = x_slot_x + (x_center_x - x_slot_x) * self.slide_t
        self._ensure_loop_texture(x_font_sp)
        if self._loop_tex is not None:
            box_x = x_lbl_x
            box_y = self.y
            box_w = x_size_w
            box_h = self.height
            tex_x = round(box_x + max(0, (box_w - self._loop_tex.width) / 2))
            tex_y = round(box_y + max(0, (box_h - self._loop_tex.height) / 2))
            origin_x = box_x + box_w / 2
            origin_y = box_y + box_h / 2
            with self.canvas.after:
                PushMatrix()
                Rotate(angle=-360.0 * self.slide_t, origin=(origin_x, origin_y))
                Color(1, 1, 1, 1)
                Rectangle(
                    texture=self._loop_tex,
                    pos=(tex_x, tex_y),
                    size=self._loop_tex.size,
                )
                PopMatrix()

    def _finish_hold(self, *_):
        if not self.pressed or self._triggered:
            return
        self._triggered = True
        if self._on_tap:
            self._on_tap()
        # Stay fully filled until reset_fill() is called (e.g. on dialog dismiss)

    def reset_fill(self, *_):
        """Animate the button back to its unfilled state."""
        self._triggered = False
        self.pressed = False
        Animation.cancel_all(self, "slide_t")
        Animation(slide_t=0.0, duration=0.18, t="out_cubic").start(self)

    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            self.pressed = True
            self._triggered = False
            Animation.cancel_all(self, "slide_t")
            anim = Animation(slide_t=1.0, duration=0.65, t="linear")
            anim.bind(on_complete=self._finish_hold)
            anim.start(self)
            return True
        return super().on_touch_down(touch)

    def on_touch_move(self, touch):
        if self.pressed:
            inside = self.collide_point(*touch.pos)
            if not inside and not self._triggered:
                self.pressed = False
                Animation.cancel_all(self, "slide_t")
                Animation(slide_t=0.0, duration=0.18, t="out_cubic").start(self)
            return True
        return super().on_touch_move(touch)

    def on_touch_up(self, touch):
        if self.pressed:
            self.pressed = False
            if not self._triggered:
                Animation.cancel_all(self, "slide_t")
                Animation(slide_t=0.0, duration=0.18, t="out_cubic").start(self)
            return True
        return super().on_touch_up(touch)


# ═══════════════════════════════════════════════════════════════════════════
# MAIN APP
# ═══════════════════════════════════════════════════════════════════════════

class SFMApp(MDApp):
    title = "Psyke"
    icon  = "icon.png"

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.state         = SanityState()
        self.fm            = FearManager()
        self.undo_stack    = UndoStack()
        self.enc_history:  list[str] = []
        self.char_name     = ""
        self.save_manager: SaveManager | None = None
        self._sep_color    = T.GOLD
        self._sep_widget:  Widget | None = None
        self._root_layout: BoxLayout | None = None
        self._base_root_padding = [0, 0, 0, 0]
        self._active_dialog: MDDialog | None = None
        self.wis_adv       = False   # Advantage on WIS saves (fear encounters)
        self.con_adv       = False   # Advantage on CON saves (wound encounters)
        self._notify_queue: list = []
        self._notify_flush_event: bool = False

    # ── Build ──────────────────────────────────────────────────────────────────

    def build(self):
        self.theme_cls.theme_style     = T.KIVY_STYLE
        self.theme_cls.primary_palette = T.KIVY_PRIMARY
        self.theme_cls.accent_palette  = T.KIVY_ACCENT

        self.save_manager = SaveManager(user_data_dir=self.user_data_dir)

        # Back button (Android hardware / ESC)
        Window.bind(on_keyboard=self._on_keyboard)

        root = FloatLayout()

        content = BoxLayout(
            orientation="vertical",
            padding=[0, 0, 0, 0],
            size_hint=(1, 1),
        )
        self._root_layout = content
        self._base_root_padding = list(content.padding)

        # Persistent header
        self._header = HeaderCard(size_hint_y=None, height=dp(182))
        content.add_widget(self._header)

        # Sanity bar
        self._san_bar = SanityBar(size_hint_y=None, height=dp(44))
        content.add_widget(self._san_bar)

        # Madness banner — kept for compat but zero height (removed per request)
        self._mad_banner = MadnessBanner(size_hint_y=None, height=0)
        # (not added to root — fully hidden)

        # Tab bar — 4 tabs
        tab_bar = BoxLayout(orientation="horizontal",
                            size_hint_y=None, height=dp(40))
        self._tab_btns: dict[str, _TabBtn] = {}
        for name, label, color in [
            ("fears",  "Fears",  T.GOLD),
            ("sanity", "Sanity", T.PURPLE),
            ("wounds", "Wounds", T.BLOOD),
            ("spells", "Spells", T.SILVER),
        ]:
            btn = _TabBtn(label, color, on_tap=lambda n=name: self._switch_tab(n))
            self._tab_btns[name] = btn
            tab_bar.add_widget(btn)
        content.add_widget(tab_bar)

        # Separator line
        self._sep_widget = Widget(size_hint_y=None, height=dp(2))
        self._sep_widget.bind(pos=self._draw_sep, size=self._draw_sep)
        content.add_widget(self._sep_widget)

        # ScreenManager
        self._sm = ScreenManager(transition=NoTransition())

        s1 = Screen(name="fears")
        self._fears_tab = FearsTab()
        s1.add_widget(self._fears_tab)
        self._sm.add_widget(s1)

        s2 = Screen(name="sanity")
        self._sanity_tab = SanityTab()
        s2.add_widget(self._sanity_tab)
        self._sm.add_widget(s2)

        s3 = Screen(name="wounds")
        self._wounds_tab = WoundsTab()
        s3.add_widget(self._wounds_tab)
        self._sm.add_widget(s3)

        s4 = Screen(name="spells")
        self._spells_tab = SpellsTab()
        s4.add_widget(self._spells_tab)
        self._sm.add_widget(s4)

        content.add_widget(self._sm)
        root.add_widget(content)

        self.session_log = SessionLog()
        self._switch_tab("fears")
        Clock.schedule_once(lambda dt: self._load())
        Clock.schedule_once(self._apply_android_safe_top_padding, 0)
        Clock.schedule_once(self._apply_android_safe_top_padding, 0.15)
        Clock.schedule_once(self._apply_android_safe_top_padding, 0.60)
        Window.bind(on_resize=lambda *_: self._apply_android_safe_top_padding())

        return root

    def _android_top_inset_px(self) -> int:
        from kivy.utils import platform as _platform

        if _platform != "android":
            return 0
        try:
            from jnius import autoclass

            PythonActivity = autoclass("org.kivy.android.PythonActivity")
            Rect = autoclass("android.graphics.Rect")

            activity = PythonActivity.mActivity
            if activity is None:
                return 0

            decor = activity.getWindow().getDecorView()

            rect = Rect()
            decor.getWindowVisibleDisplayFrame(rect)
            visible_top = int(max(0, rect.top))

            res = activity.getResources()
            res_id = res.getIdentifier("status_bar_height", "dimen", "android")
            resource_top = int(res.getDimensionPixelSize(res_id)) if res_id > 0 else 0

            return max(visible_top, resource_top)
        except Exception:
            return 0

    def _apply_android_safe_top_padding(self, *_):
        if self._root_layout is None:
            return
        top_inset = self._android_top_inset_px()
        if top_inset <= 0:
            return
        pad = list(self._base_root_padding)
        pad[1] = max(pad[1], top_inset)
        self._root_layout.padding = pad

    # ── Back button / ESC ──────────────────────────────────────────────────────

    def _on_keyboard(self, window, key, *args):
        if key == 27:  # ESC / Android back
            # Dismiss open dialog first
            if self._active_dialog:
                try: self._active_dialog.dismiss()
                except Exception: pass
                self._active_dialog = None
                return True
            # Cancel active fear encounter
            if hasattr(self, "_fears_tab") and self._fears_tab._enc.active:
                self._fears_tab.cancel_encounter()
                return True
            # Otherwise let Android handle (minimise / back to launcher)
            return False
        return False

    # ── Tab switching ──────────────────────────────────────────────────────────

    def _switch_tab(self, name: str):
        prev = self._sm.current
        self._sm.current = name
        colors = {"fears": T.GOLD, "sanity": T.PURPLE,
                  "wounds": T.BLOOD, "spells": T.SILVER}
        for n, btn in self._tab_btns.items():
            btn.set_active(n == name)
        self._sep_color = colors.get(name, T.BORDER)
        self._draw_sep()
        # Collapse all open panels when leaving or entering sanity tab
        if prev == "sanity" or name == "sanity":
            self._sanity_tab.collapse_all()

    def _draw_sep(self, *_):
        if not self._sep_widget: return
        w = self._sep_widget
        w.canvas.clear()
        with w.canvas:
            Color(*T.k(self._sep_color))
            Rectangle(pos=w.pos, size=w.size)

    # ── Load / Save ────────────────────────────────────────────────────────────

    def _load(self, *_):
        data = self.save_manager.load()
        if not data:
            self.refresh_all()
            return
        self.state.wis_score      = data.get("wis", 10)
        self.state.con_score      = data.get("con", 10)
        self.state.max_sanity     = SANITY_BASE + self.state.wis_score
        self.state.current_sanity = data.get("cur", self.state.max_sanity)
        self.state.exhaustion     = data.get("exh", 0)
        self.state.hope           = data.get("hope", False)
        self.state.hope_img_path  = data.get("hope_img", "")
        self.state.rebuild_thresholds()
        self.state.wounds    = []
        self.state.madnesses = []
        from models import WoundEntry, MadnessEntry
        for w in data.get("wounds", []):
            wound = WoundEntry.from_dict(w)
            if wound.enc_record:
                self.state.wounds.append(wound)
        for m in data.get("madnesses", []):
            self.state.madnesses.append(MadnessEntry.from_dict(m))
        self.fm.restore(data.get("fears", data))   # handle old flat format
        self.char_name = data.get("char_name", "")
        self._header._name_field.text = self.char_name
        self.enc_history = list(data.get("enc_history", []))
        for entry in self.enc_history:
            self.session_log.add_entry(entry)
        fear_enc_records = data.get("fear_enc_records", {})
        if fear_enc_records:
            self._fears_tab._restore_enc_records(fear_enc_records)
        self.refresh_all()

    # ── Global refresh ─────────────────────────────────────────────────────────

    def refresh_all(self, after_sanity=None):
        st  = self.state
        pct = st.percent * 100

        # Header chrome is cheap — update immediately
        self._san_bar.set_stage(st.madness)
        self._san_bar.set_pct(pct)
        if callable(after_sanity):
            self._san_bar.after_animation(after_sanity)
        self._mad_banner.set_stage(st.madness)
        self._header.refresh(st)

        # Spread the four heavy tab refreshes across consecutive frames so
        # they never all land in the same layout pass (eliminates the freeze).
        tabs = [
            self._fears_tab,
            self._sanity_tab,
            self._wounds_tab,
            self._spells_tab,
        ]
        def _chain(remaining):
            if not remaining:
                return
            remaining[0].refresh()
            Clock.schedule_once(lambda dt, r=remaining[1:]: _chain(r), 0)
        Clock.schedule_once(lambda dt: _chain(tabs), 0)

    def notify_event(self, message: str, tab_name: str, color_hex: str,
                     action_cb=None, extra_actions=None):
        """Queue a notification event; all events within 0.1 s are batched
        into a single card with one row per event and its own VIEW button."""
        if hasattr(self, "_san_bar") and getattr(self._san_bar, "_animating", False):
            self._san_bar.after_animation(
                lambda: self.notify_event(
                    message, tab_name, color_hex,
                    action_cb=action_cb, extra_actions=extra_actions
                )
            )
            return
        self._notify_queue.append(dict(
            message=message, tab_name=tab_name, color_hex=color_hex,
            action_cb=action_cb, extra_actions=extra_actions or []))
        if not self._notify_flush_event:
            self._notify_flush_event = True
            Clock.schedule_once(self._flush_notify, 0.08)

    def _flush_notify(self, *_):
        self._notify_flush_event = False
        events = list(self._notify_queue)
        self._notify_queue.clear()
        if not events:
            return

        # Remove any existing notification card
        for ch in list(Window.children):
            if getattr(ch, '_notify_card', False):
                Window.remove_widget(ch)

        has_any_btns = any(e['action_cb'] or e['extra_actions'] for e in events)

        card = MDCard(
            orientation="vertical",
            md_bg_color=T.k(T.BG_CARD),
            radius=[dp(8)],
            elevation=8,
            padding=0,
            spacing=0,
            size_hint_x=0.94,
            size_hint_y=None,
            adaptive_height=True,
            pos_hint={"center_x": 0.5},
            y=dp(12),
        )
        card._notify_card = True

        # ── Tab-level card color (border, header, buttons) ────────────────────
        _TAB_COLORS = {
            "fears":  T.GOLD,
            "sanity": T.PURPLE_LT,
            "wounds": T.BLOOD_LT,
            "spells": T.SILVER_LT,
        }
        tab_color = _TAB_COLORS.get(events[0]['tab_name'], T.BORDER_LT)

        # ── Header strip ─────────────────────────────────────────────────────
        hdr = MDBoxLayout(
            size_hint_y=None, height=dp(28),
            padding=[dp(12), dp(4), dp(12), dp(4)])

        def _draw_hdr(w, *_):
            w.canvas.before.clear()
            with w.canvas.before:
                Color(*T.k(tab_color, 0.18))
                Rectangle(pos=w.pos, size=w.size)
                Color(*T.k(tab_color))
                Rectangle(pos=(w.x, w.y + w.height - dp(2)),
                          size=(w.width, dp(2)))
        hdr.bind(pos=_draw_hdr, size=_draw_hdr)
        Clock.schedule_once(lambda dt: _draw_hdr(hdr))

        count_str = "1 EVENT" if len(events) == 1 else f"{len(events)} EVENTS"
        hdr.add_widget(MDLabel(
            text=count_str,
            font_style="Caption", bold=True,
            theme_text_color="Custom", text_color=T.k(tab_color),
            size_hint_x=1))
        card.add_widget(hdr)

        # ── One row per event ─────────────────────────────────────────────────
        _row_sequences = []
        for i, ev in enumerate(events):
            sep = None
            if i > 0:
                sep = Widget(size_hint_y=None, height=0)
                sep.opacity = 0
                def _sep_draw(w, *_):
                    w.canvas.clear()
                    with w.canvas:
                        Color(*T.k(T.BORDER, 0.4))
                        Rectangle(pos=w.pos, size=w.size)
                sep.bind(pos=_sep_draw, size=_sep_draw)
                Clock.schedule_once(lambda dt, s=sep: _sep_draw(s))
                card.add_widget(sep)

            row = MDBoxLayout(
                orientation="horizontal",
                size_hint_y=None, height=0,
                spacing=0, padding=[0, dp(10), 0, dp(10)])

            # Left colour accent strip
            accent = Widget(size_hint_x=None, width=dp(6))
            _col = ev['color_hex']
            def _acc(w, *_, c=_col):
                w.canvas.clear()
                with w.canvas:
                    Color(*T.k(c))
                    Rectangle(pos=w.pos, size=w.size)
            accent.bind(pos=_acc, size=_acc)
            Clock.schedule_once(lambda dt, w=accent, c=_col: _acc(w, c=c))
            row.add_widget(accent)

            row.add_widget(Widget(size_hint_x=None, width=dp(10)))

            # Message label — split into title/effects when ':' exists.
            # Title is first line (white), effects second line (keeps markup).
            _raw  = ev['message']
            if ':' in _raw:
                _colon    = _raw.index(':')
                _msg_text = (
                    f"[color={T.WHITE}]{_raw[:_colon].strip()}[/color]\n"
                    f"{_raw[_colon+1:].strip()}"
                )
            else:
                _msg_text = _raw
            _msg_text = _msg_text.replace(
                '\u2192', '[font=Symbols]\u2192[/font]')

            msg_lbl = MDLabel(
                text=_msg_text,
                markup=True,
                theme_text_color="Custom",
                text_color=(1, 1, 1, 0.92),
                font_style="Body2",
                bold=True,
                size_hint_x=1,
                size_hint_y=None,
                adaptive_height=True,
                halign="left",
                valign="top")
            msg_lbl.bind(width=lambda inst, v:
                         setattr(inst, 'text_size', (v, None)))
            row.add_widget(msg_lbl)

            # Action buttons
            all_actions = []
            if ev['action_cb']:
                all_actions.append(("VIEW >", ev['tab_name'], ev['action_cb']))
            for ex_label, ex_tab, ex_cb in ev['extra_actions']:
                all_actions.append((ex_label, ex_tab, ex_cb))

            if all_actions:
                row.add_widget(Widget(size_hint_x=None, width=dp(12)))

            view_btn = None
            for btn_i, (btn_label, btn_tab, btn_cb) in enumerate(all_actions):
                if btn_i > 0:
                    row.add_widget(Widget(size_hint_x=None, width=dp(8)))

                def _do(*_, ct=card, bt=btn_tab, bc=btn_cb):
                    if ct.parent:
                        ct.parent.remove_widget(ct)
                    self._switch_tab(bt)
                    if bc:
                        Clock.schedule_once(lambda _: bc(), 0.2)
                btn = NotificationActionButton(
                    text=btn_label,
                    color_hex=ev['color_hex'],
                    on_release=_do)
                row.add_widget(btn)
                if btn_label == "VIEW >" and ev['action_cb']:
                    view_btn = btn

            row.add_widget(Widget(size_hint_x=None, width=dp(4)))
            row.opacity = 0
            card.add_widget(row)
            _row_sequences.append((sep, row, view_btn))

        # Slide card up from below + fade in
        from kivy.utils import platform as _kp
        _notif_y = dp(64) if _kp == 'android' else dp(12)
        card.y       = _notif_y - dp(48)
        card.opacity = 0
        Window.add_widget(card)
        Animation(y=_notif_y, opacity=1, duration=0.2, t="out_cubic").start(card)

        def _show_row_sequence(idx: int, *_):
            if idx >= len(_row_sequences):
                return

            sep, row, view_btn = _row_sequences[idx]
            if sep:
                Animation(height=dp(1), opacity=1,
                          duration=0.1, t="out_cubic").start(sep)

            target_h = row.minimum_height
            if target_h <= 0:
                Clock.schedule_once(lambda dt: _show_row_sequence(idx), 0.03)
                return

            final_x = row.x
            row.x = final_x + dp(18)
            anim = Animation(
                height=target_h,
                x=final_x,
                opacity=1,
                duration=0.18,
                t="out_cubic",
            )

            def _after_row(*_):
                if view_btn:
                    view_btn.play_select_anim()
                    Clock.schedule_once(
                        lambda dt: _show_row_sequence(idx + 1),
                        0.52,
                    )
                else:
                    Clock.schedule_once(
                        lambda dt: _show_row_sequence(idx + 1),
                        0.12,
                    )

            anim.bind(on_complete=_after_row)
            anim.start(row)

        Clock.schedule_once(lambda dt: _show_row_sequence(0), 0.16)
        duration = 10.0 if has_any_btns else 6.5

        # ── Countdown bar ────────────────────────────────────────────────────
        prog_widget = Widget(size_hint_y=None, height=dp(5))
        card.add_widget(prog_widget)

        _t0      = time.perf_counter()
        _bar_col = tab_color

        def _draw_bar(dt=None):
            if not prog_widget.parent or not card.parent:
                return
            elapsed = time.perf_counter() - _t0
            frac    = max(0.0, 1.0 - elapsed / duration)
            fill_w  = card.width * frac
            prog_widget.canvas.clear()
            with prog_widget.canvas:
                Color(*T.k(T.BG_INSET))
                Rectangle(pos=prog_widget.pos,
                          size=(card.width, dp(5)))
                Color(*T.k(_bar_col, 0.85))
                Rectangle(pos=prog_widget.pos,
                          size=(fill_w, dp(5)))
            if frac > 0:
                Clock.schedule_once(_draw_bar, 1 / 25)

        Clock.schedule_once(_draw_bar, 0.05)
        Clock.schedule_once(
            lambda _: card.parent.remove_widget(card) if card.parent else None,
            duration)

    def notify_exhaustion(self, new_level: int):
        """Flash the exhaustion pip for the new level in the header."""
        self._header._exh_widget.flash_pip(new_level)

    # ── Action handlers ────────────────────────────────────────────────────────

    def confirm_reset(self):
        if self._active_dialog:
            try:
                self._active_dialog.dismiss()
            except Exception:
                pass
            self._active_dialog = None

        dlg = MDDialog(
            title="Reset Everything?",
            text=("This will clear the current character and return the app "
                  "to a brand-new state. Continue?"),
            buttons=[
                MDFlatButton(text="No", on_release=lambda *a: dlg.dismiss()),
                MDRaisedButton(
                    text="Yes",
                    md_bg_color=T.k(T.BLOOD),
                    on_release=lambda *a: self._confirm_reset_yes(dlg),
                ),
            ],
        )
        self._active_dialog = dlg

        def _on_reset_dialog_dismiss(*_):
            self._active_dialog = None
            self._header._reset_btn.reset_fill()

        dlg.bind(on_dismiss=_on_reset_dialog_dismiss)
        dlg.open()

    def _confirm_reset_yes(self, dlg):
        dlg.dismiss()
        self.reset_to_new_state()

    def reset_to_new_state(self):
        for ch in list(Window.children):
            if getattr(ch, "_notify_card", False):
                Window.remove_widget(ch)
        self._notify_queue.clear()
        self._notify_flush_event = False

        if hasattr(self, "_fears_tab") and self._fears_tab._enc.active:
            self._fears_tab.cancel_encounter()
        if hasattr(self, "_wounds_tab"):
            self._wounds_tab.reset_view()
        if hasattr(self, "_sanity_tab"):
            self._sanity_tab.collapse_all()
            self._sanity_tab._reset_add_page()

        self.state = SanityState()
        self.state.recalc_and_reset()
        self.fm = FearManager()
        self.undo_stack = UndoStack()
        self.enc_history = []
        self.char_name = ""
        self.wis_adv = False
        self.con_adv = False

        if hasattr(self, "session_log"):
            self.session_log.clear()

        self._header._name_field.text = ""
        self._switch_tab("fears")
        self.refresh_all()
        self.save_manager.save(self.state, self.fm, self.char_name, self.enc_history)

    def _on_undo(self, *_):
        if not self.undo_stack.can_undo:
            MDSnackbar(
                MDLabel(text="Nothing to undo.",
                        theme_text_color="Custom", text_color=(1, 1, 1, 1)),
                md_bg_color=T.k(T.BORDER), duration=1.5
            ).open()
            return
        snap_st, snap_fm = self.undo_stack.pop()
        self.state.restore(snap_st)
        self.fm.restore(snap_fm)
        self.refresh_all()
        self.save_manager.save(self.state, self.fm, self.char_name, self.enc_history)

    def _on_show_log(self, *_):
        dlg = MDDialog(
            title="Session Log",
            type="custom",
            content_cls=self.session_log,
            size_hint=(0.95, 0.8),
            buttons=[MDFlatButton(text="Close",
                                  on_release=lambda *a: dlg.dismiss())]
        )
        self._active_dialog = dlg
        dlg.bind(on_dismiss=lambda *_: setattr(self, "_active_dialog", None))
        dlg.open()


# ═══════════════════════════════════════════════════════════════════════════
# ENTRY POINT
# ═══════════════════════════════════════════════════════════════════════════

if __name__ == "__main__":
    SFMApp().run()
