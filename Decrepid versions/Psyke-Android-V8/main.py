...
try:
    ...
    if exc.name in {"kivy", "kivymd"}:
        python_exe = sys.executable or "python"
        raise SystemExit(
            "Missing desktop dependency: "
            f"{exc.name}\n\n"
            "Install the app dependencies, then run the app again:\n"
            f'"{python_exe}" -m pip install -r requirements-desktop.txt'
        ) from exc
    raise
...
class SFMApp(MDApp):
    ...
    def build(self):
        ...
        # Android: insert a spacer equal to the system status bar height so the
        # header isn't clipped by the notification bar at the top of the screen.
        from kivy.utils import platform as _kp
        if _kp == 'android':
            try:
                from jnius import autoclass as _jac
                _activity = _jac('org.kivy.android.PythonActivity').mActivity
                _res = _activity.getResources()
                _rid = _res.getIdentifier('status_bar_height', 'dimen', 'android')
                _sb_h = _res.getDimensionPixelSize(_rid) if _rid > 0 else int(dp(24))
            except Exception:
                _sb_h = int(dp(24))
            root.add_widget(Widget(size_hint_y=None, height=_sb_h))
    ...
    def _flush_notify(self, *_):
        ...
        for i, ev in enumerate(events):
            ...
            msg_lbl = MDLabel(
                ...
                size_hint_x=1,
                valign="middle")
            msg_lbl.bind(size=lambda inst, val:
            ...
            for btn_label, btn_tab, btn_cb in all_actions:
        ...
        # On Android, raise above the software navigation bar (~48dp)
        from kivy.utils import platform as _kp
        ...
        # Reveal each event row in sequence after the card enters
        for _i, _row in enumerate(_event_rows):
            def _show_row(dt, r=_row):
                Animation(opacity=1, duration=0.14, t="out_cubic").start(r)
            Clock.schedule_once(_show_row, 0.14 + _i * 0.1)
