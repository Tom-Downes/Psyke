...
class FearsTab(MDBoxLayout):
    ...
    def _build_encounter_card(self) -> BorderCard:
        ...
        # â”€â”€ Roll panel (hidden until encounter triggered) â”€â”€
        ...
        # Confirm row â€” [Failed Save] [Passed Save]
        ...
        def _mk_outer_card(border_hex):
            """Card: open left side (connects to encounter box), 3-sided border,
            tinted background in the card's own colour."""
            box = MDBoxLayout(
                orientation="vertical", spacing=dp(6),
                padding=[dp(8), dp(10), dp(10), dp(10)],
                adaptive_height=True)
            with box.canvas.before:
                # Tinted fill â€” subtle shade of the card colour
                Color(*T.k(border_hex, 0.13))
                _bg = RoundedRectangle(radius=[0, dp(6), dp(6), 0])
                # 3-sided border: top â†’ right â†’ bottom  (left side open)
                Color(*T.k(border_hex))
                _line = Line(width=dp(1.5))
            def _upd(w, *_, _bg=_bg, _line=_line):
                _bg.pos  = w.pos
                _bg.size = w.size
                x, y, ww, hh = w.x, w.y, w.width, w.height
                _line.points = [x, y + hh,  x + ww, y + hh,
                                x + ww, y,   x, y]
    ...
    def _build_severity_section(self) -> BorderCard:
        ...
        for s in range(1, 5):
            ...
            inner.add_widget(CaptionLabel(
                sev_label,
                color_hex=info.color, height_dp=18))
            _desc = MDLabel(
                text=info.desc,
    ...
    def _build_desens_section(self) -> BorderCard:
        ...
        for r in range(1, 5):
            ...
            row = ListItem(
                ...
                secondary=f"Level {r}  |  DC {DESENS_DC[r]}",
                accent_hex=color,
    ...
    def _show_roll_panel(self):
        panel = self._roll_panel
        panel.size_hint_y = None
        panel.height  = 0
        panel.opacity = 0
        # Reset sub-widget states (instant â€” panel is invisible)
        ...
        self._confront_card.size_hint_x = 1
        self._avoid_card.size_hint_x    = 1
        ...
        # One frame later: measure then animate open
        def _do_anim(dt):
            target_h = panel.minimum_height
            Animation(height=target_h, opacity=1, duration=0.18, t="out_quart").start(panel)
    ...
    def _on_encounter(self, *_):
        ...
        _sev_title = f"{info.name} + Exhaustion" if stage == 4 else info.name
        _sev_msg = f"{_sev_title}: [color={info.color}]{info.desc}[/color]"
        app.notify_event(
            _sev_msg,
            "fears", info.color,
            action_cb=lambda st=stage: self.open_severity(st))
    ...
    def _use_hope(self):
        ...
        self._hope_row.opacity = 0
        app.refresh_all()
        # Hope guarantees a pass â€” auto-succeed the encounter
        self._on_pass()

    # â”€â”€ Event: Confirm failed save â€” Step 2 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _on_confirm_fail(self, *_):
        """Player confirms the failed save â€” roll sanity dice and show preview."""
        app   = self._app()
        ...
        self._log(f"Failed save â€” sanity roll: {info.dice}d4 ({rt}) = {total}")
        ...
        # Helpers â€” short names and arrow token
        ...
        # Fade confirm row out, then animate cards emerging from left
        Animation(height=0, opacity=0, duration=0.1, t="out_cubic").start(self._confirm_row)

        self._push_btn.disabled  = True   # re-enabled after animation
        self._avoid_btn.disabled = True
        self._save_confirmed     = True

        def _reveal_preview(dt):
            # Measure natural card heights at full width before collapsing.
            # If layout hasn't settled yet (heights still 0) defer one frame.
            conf_h  = self._confront_card.minimum_height
            avoid_h = self._avoid_card.minimum_height
            if conf_h == 0 or avoid_h == 0:
                Clock.schedule_once(_reveal_preview, 0)
                return
            spacing = dp(8)

            # Target pixel width: preview_box inner width (+22dp for the -11dp
            # left/right negative padding that bleeds cards past the container).
            target_w = max(
                self._confront_card.width,
                self._preview_box.width + dp(22),
                dp(50),
            )

            # Break the adaptive_height binding (minimum_height → height) on
            # both cards before collapsing their width.  Without this, setting
            # width=1 causes label text to reflow to 1 px wide, minimum_height
            # shoots up to a huge value, and the binding immediately overrides
            # the pinned height — creating the "long height glitch".
            cc, ac = self._confront_card, self._avoid_card
            cc.unbind(minimum_height=cc.setter('height'))
            ac.unbind(minimum_height=ac.setter('height'))

            for card, h in ((cc, conf_h), (ac, avoid_h)):
            ...
            # Show preview_box at its full height so the roll panel doesn't jump
            self._preview_box.size_hint_y = None
            self._preview_box.height  = conf_h + avoid_h + spacing
            self._preview_box.opacity = 1
            Clock.schedule_once(lambda dt2: self._sync_panel_height(self._roll_panel), 0)

            # Both cards grow simultaneously from the left edge at matching speed
            def _show_conf(dt2):
                cc.opacity = 1
                Animation(width=target_w, duration=0.35, t="out_quart").start(cc)

            def _show_avoid(dt2):
                ac.opacity = 1
                anim_a = Animation(width=target_w, duration=0.35, t="out_quart")
                def _on_done(*_):
                    # Restore adaptive_height binding and relative sizing
                    cc.bind(minimum_height=cc.setter('height'))
                    ac.bind(minimum_height=ac.setter('height'))
                    cc.size_hint_x = 1
                    ac.size_hint_x = 1
                    self._sync_panel_height(self._preview_box)
                    Clock.schedule_once(lambda dt3: self._sync_panel_height(self._roll_panel), 0)
                    self._push_btn.disabled  = False
                    self._avoid_btn.disabled = False
                anim_a.bind(on_complete=_on_done)
                anim_a.start(ac)

            Clock.schedule_once(_show_conf,  0.04)
            Clock.schedule_once(_show_avoid, 0.04)

        Clock.schedule_once(_reveal_preview, 0.12)

    # â”€â”€ Event: Pass â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    def _on_pass(self, *_):
        self._log("Passed â€” encounter ends cleanly.")
        self._end_enc()
        self._save()
