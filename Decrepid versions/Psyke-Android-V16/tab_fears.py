...
from hope_decision_toggle import HopeDecisionToggle
import theme as T
...
class _EncTab(ButtonBehavior, MDBoxLayout):
    ...
    def __init__(self, key: str, label: str, on_tap, **kwargs):
        ...
        self._commit_pause_at_top = False
        self._commit_paused = False
        self._commit_top_reached = False
        self._commit_on_top_cb = None
        self._cap_hex        = None
    ...
    def play_commit_anim(
        ...
        self._commit_pause_at_top = bool(pause_at_top)
        self._commit_paused       = False
        self._commit_top_reached  = False
        self._commit_on_top_cb    = on_top_reached
        self._fill_col.rgba       = (*T.k(self._main_hex)[:3], 0.18)
        self._accent_col.rgba     = (*T.k(self._main_hex)[:3], 0.28)
        self._text_col_instr.rgba = (*T.k(self._main_hex)[:3], 0.70)
        self._apply_cap_state(0.0)
    ...
    def resume_commit_anim(self, on_complete=None):
    ...
    def _tick_commit_stroke(self, dt):
        ...
        if (
            ...
            top_stop_prog = top_phase_dur
        else:
            top_stop_prog = trace_end * (top_len / total_path) if total_path > 0 else 0.0

        if (
            self._commit_pause_at_top
            and not self._commit_top_reached
            and self._commit_prog >= top_stop_prog
        ):
            self._commit_prog = top_stop_prog
            self._commit_top_reached = True
            self._commit_paused = True
            cb = self._commit_on_top_cb
            self._commit_on_top_cb = None
            if cb:
                Clock.schedule_once(lambda _dt: cb(), 0)
        ...
        if self._commit_paused:
            stroke_alpha = 1.0
        elif self._commit_prog >= hold_end and total > hold_end:
...
class EncounterListItem(MDBoxLayout):
    ...
    def set_open(self, open_state: bool, animate: bool = True, on_complete=None):
        ...
        if open_state:
            ...
            close_hooks = [
                getattr(child, "_on_parent_close", None)
                for child in self._detail_box.children
                if callable(getattr(child, "_on_parent_close", None))
            ]
            if close_hooks:
                pending = {"count": len(close_hooks), "started": False}
                ...
                def _invoke_close(_cb):
                    try:
                        _cb(on_done=_child_done)
                    except TypeError:
                        _cb()
                        _child_done()

                for cb in close_hooks:
                    Clock.schedule_once(lambda dt, _cb=cb: _invoke_close(_cb), 0)
            else:
                _run_close_anim()
...
class FearsTab(MDBoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self._awaiting_hope_choice = False
    ...
    def _hope_toggle_gesture_active(self, touch=None) -> bool:
        toggle = getattr(self, "_hope_toggle", None)
        row = getattr(self, "_hope_row", None)
        if not toggle or not row or toggle.parent is None:
            return False
        if row.opacity <= 0 or row.height <= 0:
            return False
        if touch is not None and touch.ud.get("hope_toggle_touch"):
            return True
        if getattr(toggle, "dragging", False):
            return True
        if touch is not None and toggle.collide_point(*touch.pos):
            return True
        return False
    ...
    def on_touch_down(self, touch):
        if getattr(self, "_hope_toggle", None) and self._hope_toggle.collide_point(*touch.pos):
            touch.ud["hope_toggle_touch"] = True
        if self._hope_toggle_gesture_active(touch):
            return super().on_touch_down(touch)
    ...
    def on_touch_up(self, touch):
        if self._hope_toggle_gesture_active(touch):
            touch.ud.pop("hope_toggle_touch", None)
    ...
    def _build_encounter_section(self) -> MDBoxLayout:
        ...
        enc_btn_wrap = FloatLayout(size_hint_x=None, width=dp(132), size_hint_y=None, height=dp(52))
        self._enc_btn.pos_hint = {"center_x": 0.5, "center_y": 0.5}
        enc_btn_wrap.add_widget(self._enc_btn)
        dc_row.add_widget(enc_btn_wrap)
        ...
        # Hope row — hidden until a fail result with active hope
        self._hope_row = MDBoxLayout(
            size_hint_y=None, height=0, opacity=0)
        self._hope_toggle = HopeDecisionToggle(
            on_accept_failure=lambda _w: self._decline_hope(),
            on_use_hope=lambda _w: self._use_hope(),
        )
        self._hope_row.add_widget(self._hope_toggle)
        ...
        self._enc_delete_btn = MDIconButton(
            ...
            disabled_color=T.k(T.RED),
            size_hint_x=None, width=dp(40),
            disabled=True,
            on_release=self._on_delete_selected_encounter)
        enc_delete_wrap = FloatLayout(size_hint_x=None, width=dp(40), size_hint_y=None, height=dp(32))
        self._enc_delete_btn.pos_hint = {"center_x": 0.5, "center_y": 0.43}
        enc_delete_wrap.add_widget(self._enc_delete_btn)
        self._enc_header_row.add_widget(enc_delete_wrap)
    ...
    def _set_stage_available(self, key: str, available: bool = True,
        ...
        if not available:
            ...
            tab._commit_pause_at_top = False
            tab._commit_paused   = False
            tab._commit_top_reached = False
            tab._commit_on_top_cb = None
    ...
    def _open_encounter_stage(self, key: str, animate: bool = True):
        ...
        if self._awaiting_hope_choice and key != "save":
            return
        panels = {
            "severity": self._severity_stage_box,
            "save":   self._save_stage_box,
    ...
    def _reset_enc_ui(self):
        ...
        self._hope_toggle.reset(animate=False)
    ...
    def _save_outcome_hex(self, passed: bool) -> str:
        return T.GREEN if passed else T.BLOOD

    def _show_hope_choice(self):
        if not self._enc.active:
            return
        self._awaiting_hope_choice = True
        self._outcome_lbl.text = "FAILED"
        self._outcome_lbl.text_color = T.k(T.BLOOD)
        self._outcome_lbl.opacity = 1
        self._hope_toggle.reset(animate=False)
        self._hope_row.height = dp(120)
        self._hope_row.opacity = 1
        Clock.schedule_once(lambda dt: self._sync_shell_height(), 0.02)

    def _finalize_save_choice(self, passed: bool, after_choice):
        save_tab = self._enc_stage_tabs.get("save")
        if save_tab is None:
            return
        self._awaiting_hope_choice = False
        self._hope_row.height = 0
        self._hope_row.opacity = 0
        save_tab._cap_hex = self._save_outcome_hex(passed)
        Animation(text_color=T.k(save_tab._cap_hex), duration=0.20, t="out_quad").start(self._enc_roll_num)
        self._outcome_lbl.text = "PASSED" if passed else "FAILED"
        self._outcome_lbl.text_color = T.k(save_tab._cap_hex)
        self._outcome_lbl.opacity = 0
        Animation(opacity=1, duration=0.22, t="out_cubic").start(self._outcome_lbl)
        Clock.schedule_once(lambda dt: self._sync_shell_height(), 0.02)
        save_tab.resume_commit_anim(
            on_complete=lambda: Clock.schedule_once(after_choice, 0.10)
        )
    ...
    def _build_frozen_encounter_shell(self, record: dict):
        ...
        # ── save panel ───────────────────────────────────────────────────
        save_col = T.GREEN if passed else T.BLOOD
    ...
    def _animate_roll_result(self, wis_save: int, dc: int):
        """WIS save spin + wrap run together, pausing at the corner if Hope can intervene."""
        passed = wis_save >= dc
        final_hex = self._save_outcome_hex(passed)
        final_rgba = T.k(final_hex)
        has_hope_choice = (not passed and self._app().state.hope)
        ...
        self._hope_row.height         = 0
        self._hope_row.opacity        = 0
        self._enc_stage_tabs["save"]._cap_hex = None

        def _show_final_outcome():
            self._enc_stage_tabs["save"]._cap_hex = final_hex
            self._outcome_lbl.text = "PASSED" if passed else "FAILED"
        ...
        def _after_save_commit():
            if passed:
                Clock.schedule_once(self._on_pass, 0.10)
            else:
                Clock.schedule_once(self._on_confirm_fail, 0.10)
        ...
        def _land(dt):
            ...
            Clock.schedule_once(lambda dt: self._sync_shell_height(), 0.02)
            if not has_hope_choice:
                Clock.schedule_once(lambda _dt: _show_final_outcome(), 0)
        ...
        post_top_dur = normal_trace_dur * ((total_path - top_w) / total_path)
        top_phase_dur = _spin_t + 0.30
        trace_dur = top_phase_dur + post_top_dur

        self._play_tab_commit(
            "save",
            total_duration=trace_dur,
            on_complete=None if has_hope_choice else _after_save_commit,
            trace_duration=trace_dur,
            top_phase_duration=top_phase_dur,
            pause_at_top=has_hope_choice,
            on_top_reached=self._show_hope_choice if has_hope_choice else None,
        )
    ...
    def _use_hope(self):
        if not self._awaiting_hope_choice:
            return
        app = self._app()
        self._push_undo()
        ...
        self._finalize_save_choice(True, self._on_pass)

    # â"€â"€ Event: Confirm failed save — Step 2 â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€

    def _decline_hope(self):
        if not self._awaiting_hope_choice:
            return
        self._finalize_save_choice(False, self._on_confirm_fail)
