...
class _EncTab(ButtonBehavior, MDBoxLayout):
    ...
    def set_state(self, state: str):
        ...
        # Sweep completed â†' locked to solid gold + stage-coloured side tab
        if self._commit_done:
            if state == "active":
                ...
                self._lbl.text_color  = (*T.k(T.GOLD_LT)[:3], 0.70)
            elif state == "done_new":
...
class EncounterListItem(MDBoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            ...
            self._accent_color = Color(*T.k(T.GOLD))
        ...
        self._text_col = MDBoxLayout(
            ...
            size_hint_y=None, adaptive_height=True)
        self._title_stage = FillSwipeTitle(fill_rgba=list(T.k(T.GOLD)))
    ...
    def _redraw(self, *_):
        ...
        self._accent_color.rgba = T.k(T.GOLD)
    ...
    def start_live(self, fear_name: str, stage: int, flow_shell, subtitle: str = ""):
        ...
        self._detail_box.add_widget(flow_shell)
    ...
    def set_completed(self, title: str, subtitle: str, content_widget, on_complete=None):
        ...
        self._subtitle_lbl.text = subtitle
        self._detail_box.clear_widgets()
        self._detail_box.add_widget(content_widget)
    ...
    def flash(self):
        ...
        with self.canvas.after:
            self._stroke_col  = Color(*T.k(T.GOLD), 1.0)
    ...
    def _tick_stroke(self, dt):
        ...
        self._stroke_col.rgba = (*T.k(T.GOLD)[:3], alpha)
...
class FearsTab(MDBoxLayout):
    ...
    def _build_frozen_encounter_shell(self, record: dict):
        ...
        def _mk_card(border_hex, min_h=100):
            ...
            def _upd(w, *_, _bd=_bd, _bg=_bg, _ln=_ln):
                reveal_t = max(0.0, min(1.0, getattr(w, "_reveal_t", 1.0)))
                vis_w = max(0, w.width * reveal_t)
        ...
        if not passed and choice in ('confront', 'avoid'):
            if choice == 'confront':
                ...
                choice_panel.add_widget(
                    _mk_metric('SANITY LOSS', f"\u2212{record.get('san_change', 0)}", T.PURPLE_LT))
                if record.get('fear_removed'):
                    choice_panel.add_widget(
                        _mk_metric('DESENSITIZATION', 'Fear removed', T.DESENS_LT))
                else:
                    dn_old = DESENS_NAMES[record.get('desens_old', 0)].split()[0]
                    dn_new = DESENS_NAMES[record.get('desens_new', 0)].split()[0]
                    choice_panel.add_widget(
                        _mk_metric('DESENSITIZATION', f'{dn_old} \u2192 {dn_new}  +1', T.DESENS_LT))
            else:
                choice_panel = _mk_card(T.STAGE_1, 150)
                choice_panel.add_widget(
                    _lbl('AVOID', color=T.GREEN, style='Subtitle1', bold=True, h=28))
                choice_panel.add_widget(
                    _mk_metric('SANITY GAIN', f"+{record.get('san_change', 0)}", T.PURPLE_LT))
                sn_old = FEAR_STAGES[record.get('sev_old', 1)].name
                sn_new = FEAR_STAGES.get(record.get('sev_new', 1), FEAR_STAGES[4]).name
                choice_panel.add_widget(
                    _mk_metric('SEVERITY', f'{sn_old} \u2192 {sn_new}  +1', T.GOLD_LT))
                d_old = record.get('desens_old', 0)
                d_new = record.get('desens_new', 0)
                if d_old != d_new:
                    dn_o = DESENS_NAMES[d_old].split()[0]
                    dn_n = DESENS_NAMES[d_new].split()[0]
                    choice_panel.add_widget(
                        _mk_metric('DESENSITIZATION', f'{dn_o} \u2192 {dn_n}  \u22121', T.DESENS_LT))
                else:
                    choice_panel.add_widget(
                        _mk_metric('DESENSITIZATION', 'Already at minimum', T.DESENS_LT))
    ...
    def _switch_encounter_list(self, new_name: str | None):
        ...
        if self._enc_list_fear == new_name:
            if new_name:
                self._enc_section_lbl.text     = f"{new_name} Encounter List"
