...
class FearsTab(MDBoxLayout):
    ...
    def _on_confirm_fail(self, *_):
        ...
        def _reveal_preview(dt):
            ...
            # Collapse cards to zero width (left-anchored in BoxLayout)
            for card, h in ((self._confront_card, conf_h), (self._avoid_card, avoid_h)):
                card.size_hint_x = 0
            ...
            def _show_conf(dt2):
                self._confront_card.opacity = 1
                Animation(size_hint_x=1, duration=0.35, t="out_quart").start(self._confront_card)

            def _show_avoid(dt2):
                self._avoid_card.opacity = 1
                anim_a = Animation(size_hint_x=1, duration=0.35, t="out_quart")
                def _on_done(*_):
