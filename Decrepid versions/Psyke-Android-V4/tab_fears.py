...
class FearsTab(MDBoxLayout):
    ...
    def _on_severity_select(self, s: int):
        ...
        if self._selected_fear:
            ...
            info  = FEAR_STAGES[s]
            color = info.color
            prefix = "EXTREME - " if s == 4 else ""
            app.notify_event(
                f"{prefix}{name}: Severity > {info.name}",
                "fears", color,
                action_cb=lambda _s=s: self.open_severity(_s)
            )
    ...
    def _on_desens_select(self, rung: int):
        ...
        self._save()
        color = DESENS_RUNG_COLORS[rung]
        app.notify_event(
            f"{name}: Desensitization > {DESENS_NAMES[rung]}",
            "fears", color,
            action_cb=lambda r=rung: self.open_desens(r)
        )
    ...
    def _on_push(self, *_):
        ...
        if fear_cured:
            ...
            Clock.schedule_once(
                lambda _, r=new_rung, n=name, a=amt, c=cur, m=mx: app.notify_event(
                    f"Confront: {n} - Lost {a} sanity ({c}/{m})",
                    "fears", T.BLOOD,
                    action_cb=lambda nn=n: self.open_fear(nn),
                    extra_actions=[("DESENS >", "fears", lambda rr=r: self.open_desens(rr))]
                ), 0.15)
        ...
        if fear_cured:
            def _cure_fear(_):
                ...
                self.refresh()
                self._end_enc()
            Clock.schedule_once(_cure_fear, 3.0)
        else:
            Clock.schedule_once(lambda _: self._end_enc(), 3.0)
        self._save()
    ...
    def _on_avoid(self, *_):
        ...
        # Fire avoid notification with severity + desens buttons (delay to follow panic if any)
        _delay = 0.5 if stage == 4 else 0.15
        Clock.schedule_once(
            lambda _, r=new_rung, s=new_stage, n=name, a=amt, c=cur, m=mx: app.notify_event(
    ...
    def _handle_thresholds(self, threshs):
        ...
        for label, kind in threshs:
            ...
            if existing:
                ...
                app.notify_event(f"{label}: {m.name}", "sanity", color,
                                 action_cb=lambda e=m: sanity_tab.open_madness(e))
        if threshs:
            app.refresh_all()
