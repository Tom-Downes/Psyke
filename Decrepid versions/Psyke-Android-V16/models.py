...
DESENS_DESCS: Dict[int, str] = {
    1: "Level 1 | Encounter DC 16",
    2: "Level 2 | Encounter DC 14",
    3: "Level 3 | Encounter DC 12",
    4: "Level 4 | Encounter DC 10",
}
...
DESENS_RUNG_COLORS: Dict[int, str] = {
    1: "#b8eeff",   # Low      — light sky blue
    2: "#72b8f0",   # Moderate — medium light blue
    3: "#3d7fd4",   # High     — medium blue
    4: "#1a4ecc",   # Extreme  — deep blue
}
...
FEAR_RULES_TEXT = (
    ...
    "  Level 3 — Disadvantage on Attack Rolls & Saving Throws\n"
...
WOUND_RULES_TEXT = (
    ...
    "─────────────────────\n\n"
    "EXHAUSTION\n"
    "Exhaustion accumulates from wound check failures by 5 or more (+1), "
    "Extreme Severity fear encounters (+1), and certain madness effects.\n\n"
    "Exhaustion Levels:\n"
    "  Level 1 — Disadvantage on Ability Checks\n"
    "  Level 2 — Speed Halved\n"
    "  Level 3 — Disadvantage on Attack Rolls & Saving Throws\n"
...
FEAR_STAGES: Dict[int, FearStageInfo] = {
    1: FearStageInfo(
        "Low Severity",
        "You have disadvantage on ability checks related to the source of your fear.",
        1, "#50a870"),
    2: FearStageInfo(
        "Moderate Severity",
        "You are frightened and cannot take bonus actions.",
        2, "#c8a44e"),
    3: FearStageInfo(
        "High Severity",
        "You are frightened and incapacitated until the end of your next turn.\n"
        "After that, you have disadvantage on attack rolls, ability checks, and saving throws for the rest of the encounter.",
        3, "#d08040"),
    4: FearStageInfo(
        "Extreme Severity",
        "You are frightened. You fall prone and become unconscious (stable) for up to 1 minute, or until an ally uses an action to rouse you.\n"
        "At the end of the encounter, you gain 1 level of exhaustion.\n"
        "At extreme severity, you also gain a new random fear.",
        4, "#c44040"),
}
...
def roll_insanity_duration(kind: str) -> str:
    """Roll random duration: 1d10 min (short), 1d10×10 hr (long), Until cured (indefinite)."""
    if kind == "short":
...
@dataclass
class MadnessEntry:
    kind: str; roll_range: str; effect: str; timestamp: str = ""; name: str = ""; duration: str = ""
...
class WoundEntry:
    ...
    enc_record: dict = field(default_factory=dict)

    def to_dict(self):
        d = {"description": self.description, "effect": self.effect,
             "severity": self.severity, "timestamp": self.timestamp}
        if self.enc_record:
            d["enc_record"] = self.enc_record
        return d
    ...
    def from_dict(d: dict) -> WoundEntry:
        return WoundEntry(description=d.get("description","Unknown wound"),
                          ...
                          timestamp=d.get("timestamp",""),
                          enc_record=d.get("enc_record") or {})
...
class SanityState:
    ...
    @property
    def encounter_wounds(self): return [w for w in self.wounds if w.enc_record]
    @property
    def encounter_minor_wounds(self):
        return [w for w in self.encounter_wounds if w.severity == "minor"]
    @property
    def encounter_major_wounds(self):
        return [w for w in self.encounter_wounds if w.severity == "major"]
    ...
    def apply_recovery(self, amt: int):
        """Restore sanity; return list of (label, threshold, kind) for thresholds cleared upward."""
        old_fired = set(self.fired_thresholds)
        self.current_sanity = min(self.max_sanity, self.current_sanity + max(0, amt))
        self.rebuild_thresholds()
        # Thresholds that were fired but are no longer (sanity recovered above them)
        cleared = [
            (label, c, kind)
            for label, c, kind in THRESHOLDS
            if c in old_fired and c not in self.fired_thresholds
        ]
        return cleared
    ...
    def add_wound(self, desc, effect, severity) -> WoundEntry:
        ...
        self.wounds.insert(0, w); return w
    ...
    def add_madness(self, kind: str, custom_effect: str = "") -> MadnessEntry:
        ...
        dur = roll_insanity_duration(kind)
        if custom_effect:
...
class FearManager:
    def __init__(self):
        ...
        self.enc_counts: Dict[str, int] = {}  # fear name → lifetime encounter count
    ...
    def remove(self, n: str):
        ...
        self.enc_counts.pop(n, None)
        return bool(self.fears.pop(n, None))

    def get_enc_count(self, name: str) -> int:
        return self.enc_counts.get(name, 0)

    def incr_enc_count(self, name: str) -> int:
        n = self.enc_counts.get(name, 0) + 1
        self.enc_counts[name] = n
        return n
    ...
    def snapshot(self) -> dict:
        return {
            "fears": dict(self.fears),
            "desens": dict(self.desens),
            "enc_counts": dict(self.enc_counts),
        }
    ...
    def restore(self, s):
        ...
        self.enc_counts = {str(k): max(0, int(v))
                           for k, v in s.get("enc_counts", {}).items()}
...
class SaveManager:
    ...
    def save(self, st: SanityState, fm: FearManager, char_name: str, enc_history: list,
        ...
        data = {"wis": st.wis_score, "con": st.con_score, "cur": st.current_sanity,
                ...
                "wounds": [w.to_dict() for w in st.encounter_wounds],
                "madnesses": [m.to_dict() for m in st.madnesses],
                "fear_enc_records": fear_enc_records or {}}
        try:
