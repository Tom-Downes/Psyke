...
class _HopePortrait(BoxLayout):
    """
    Circular character portrait with a 'Hope' toggle.
    Short tap  = toggle active (red border) / inactive (dim border).
    Long press = open image picker to choose a portrait from gallery / files.
    """

    def __init__(self, on_toggle, on_image_picked, **kwargs):
        kwargs.setdefault("orientation", "vertical")
        kwargs.setdefault("size_hint_x", None)
        ...
        self._active        = False
        self._on_toggle     = on_toggle
        self._on_img_picked = on_image_picked
        self._img_texture   = None
        self._loaded_path   = ""

        # Canvas-drawn circle widget
        self._port = Widget(size_hint_y=None, height=dp(40))
    ...
    def _redraw(self, *_):
        ...
        with w.canvas:
            ...
            id_ = d - bw * 2
            ix, iy = x + bw, y + bw
            if self._img_texture:
                Color(1, 1, 1, 1)
                Ellipse(texture=self._img_texture, pos=(ix, iy), size=(id_, id_))
            else:
                # Dark fill
                Color(*T.k(T.BG_INSET))
                Ellipse(pos=(ix, iy), size=(id_, id_))
                # Silhouette: head circle + body oval
                Color(*T.k(c, 0.65))
                cx  = ix + id_ / 2
                hr  = id_ * 0.22
                Ellipse(pos=(cx - hr, iy + id_ * 0.47), size=(hr * 2, hr * 2))
                bw2 = id_ * 0.30; bh2 = id_ * 0.32
                Ellipse(pos=(cx - bw2 / 2, iy + dp(2)), size=(bw2, bh2))

    def set_state(self, active: bool, img_path: str = ""):
        self._active = active
        self._lbl.text_color = T.k(T.BLOOD_LT if active else T.TEXT_DIM)
        self._lbl.bold       = active
        if img_path and img_path != self._loaded_path:
    ...
    def _load_image(self, path: str):
        try:
            ...
            self._img_texture = CoreImage(path).texture
    ...
    def _pick_image(self):
        try:
            ...
            try:
                ...
                root.destroy()
                if path:
                    Clock.schedule_once(lambda dt: self._on_selected([path]))
            except Exception:
                pass
    ...
    def _on_selected(self, selection):
        ...
        self._on_img_picked(path)

    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            touch.ud['_hope_t'] = Clock.get_time()
            return True
        return super().on_touch_down(touch)

    def on_touch_up(self, touch):
        if self.collide_point(*touch.pos) and '_hope_t' in touch.ud:
            elapsed = Clock.get_time() - touch.ud['_hope_t']
            if elapsed >= 0.5:
                self._pick_image()
            else:
                self._active = not self._active
...
class HeaderCard(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        kwargs.setdefault("height", dp(168))
        kwargs.setdefault("spacing", dp(2))
    ...
    def _build(self):
        # Row 1 — adventure name + hope portrait + sanity chip (58dp)
        row1 = MDBoxLayout(size_hint_y=None, height=dp(58), spacing=dp(6))
        ...
        row1.add_widget(self._hope_portrait)
        self._san_chip = _SanityChip(size_hint_y=None, height=dp(54))
        row1.add_widget(self._san_chip)
        self.add_widget(row1)

        # ADV row — WIS ADV toggle | CON ADV toggle | spacer (20dp)
        ...
        # Row 2 — WIS chip + CON chip + exhaustion (72dp)
        ...
        exh_desc_box = BoxLayout(orientation="vertical", size_hint_y=None, height=dp(16))
        ...
        exh_area.add_widget(exh_desc_box)
...
class SFMApp(MDApp):
    ...
    def build(self):
        ...
        # Persistent header
        self._header = HeaderCard(size_hint_y=None, height=dp(168))
