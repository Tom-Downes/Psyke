...
- Stores the base app version (`14.0.0`) plus the numeric Android version code that the CI script updates per build.
...
- Runs `buildozer -v android release`, tees the full log to `buildozer-aab.log`, signs the resulting `.aab` with `jarsigner`, verifies the signature, and copies the final artifact to `ABB/ABB-V14.aab`.
