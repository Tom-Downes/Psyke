...
class SFMApp(MDApp):
    ...
    def _flush_notify(self, *_):
        ...
        # ── Header strip ─────────────────────────────────────────────────────
        hdr_color = events[0]['color_hex'] if len(events) == 1 else T.GOLD
        hdr = MDBoxLayout(
            size_hint_y=None, height=dp(22),
        ...
        hdr.add_widget(MDLabel(
            text=count_str,
            font_style="Caption", bold=True,
            theme_text_color="Custom", text_color=T.k(hdr_color),
            size_hint_x=1))
        card.add_widget(hdr)
        ...
        for i, ev in enumerate(events):
            ...
            accent = Widget(size_hint_x=None, width=dp(5))
            _col = ev['color_hex']
            def _acc(w, *_, c=_col):
                w.canvas.clear()
                with w.canvas:
        ...
        duration = 7.0 if has_any_btns else 4.5
        Clock.schedule_once(
