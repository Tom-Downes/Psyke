...
class WoundsTab(MDBoxLayout):
    ...
    def _on_wound_encounter(self, *_):
        ...
        self._w_roll_panel.height  = _ROLL_H + (dp(116) if show_hope else 0)
    ...
    def _use_hope(self):
        ...
        app.refresh_all()
        # Hope guarantees a pass — auto-resolve as no wound
        self._resolve("pass5")

    def _end_enc(self):
        self._enc.reset()
        self._w_roll_panel.height  = 0
        self._w_roll_panel.opacity = 0
        self._w_result_lbl.text    = ""
    ...
    def _resolve(self, outcome: str):
        ...
        if outcome == "pass5":
            ...
            app.refresh_all()
            app.notify_event(f"Minor Wound: {desc}", "wounds", T.BLOOD,
                             action_cb=lambda e=_we: self.open_wound(e, "minor"))
        elif outcome == "fail":
            ...
            app.refresh_all()
            app.notify_event(f"Major Wound: {desc}", "wounds", T.BLOOD,
                             action_cb=lambda e=_we: self.open_wound(e, "major"))
        elif outcome == "fail5":
            _, desc, effect = roll_random_wound("major")
            _we = app.state.add_wound(desc, effect, "major")
            ...
            app.refresh_all()
            app.notify_event(f"Major Wound + Exhaustion: {desc}", "wounds", T.BLOOD,
                             action_cb=lambda e=_we: self.open_wound(e, "major"))
            _exh = app.state.exhaustion
            Clock.schedule_once(lambda _: app.notify_exhaustion(_exh), 0.5)

        Clock.schedule_once(lambda _: self._end_enc(), 0.8)
        self._save()
    ...
    def _add_wound(self, severity: str, desc: str = "", effect: str = ""):
        ...
        Clock.schedule_once(
            lambda _, e=entry, s=severity: app.notify_event(
                f"{severity.title()} Wound: {desc}",
                "wounds", color,
