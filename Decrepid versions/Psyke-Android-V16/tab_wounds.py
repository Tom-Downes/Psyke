...
class WoundsTab(MDBoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Stored callback for the hope-decline path (set in _on_wound_encounter)
        self._pending_after_save = None

        # Hope decision state (mirrors fear encounter)
        self._awaiting_hope_choice: bool = False
        # hope_row and hope_toggle are created inside _build_save_stage()
        self._hope_row    = None
        self._hope_toggle = None
    ...
    def _build_encounter_section(self) -> BorderCard:
        ...
        self._dmg_field.bind(text=lambda *_: self._sync_wound_encounter_button())
        ...
        wenc_btn_wrap = FloatLayout(size_hint_x=None, width=dp(132), size_hint_y=None, height=dp(52))
        self._wenc_btn.width = dp(132)
        self._wenc_btn.pos_hint = {"center_x": 0.5, "center_y": 0.5}
        wenc_btn_wrap.add_widget(self._wenc_btn)
        field_row.add_widget(wenc_btn_wrap)
        card.add_widget(field_row)
        Clock.schedule_once(lambda *_: self._sync_wound_encounter_button(), 0)
        ...
        enc_rm_btn = MDIconButton(
            icon="trash-can-outline",
            theme_icon_color="Custom", icon_color=T.k(T.RED),
            size_hint_x=None, width=dp(40),
            on_release=self._on_remove_enc)
        enc_rm_btn_wrap = FloatLayout(size_hint_x=None, width=dp(40), size_hint_y=None, height=dp(32))
        enc_rm_btn.pos_hint = {"center_x": 0.5, "center_y": 0.43}
        enc_rm_btn_wrap.add_widget(enc_rm_btn)
        enc_list_hdr.add_widget(enc_rm_btn_wrap)
    ...
    def _sync_wound_encounter_button(self, *_):
        if not hasattr(self, "_wenc_btn") or self._wenc_btn is None:
            return
        try:
            dmg = safe_int(getattr(self._dmg_field, "text", "") or "0", lo=0)
        except Exception:
            dmg = 0
        self._wenc_btn.disabled = dmg <= 0
    ...
    def _build_save_stage(self):
        ...
        self._hope_row = MDBoxLayout(size_hint_y=None, height=0, opacity=0)
    ...
    # ── Hope decision (wound save) ────────────────────────────────────────────

    def _show_hope_choice(self):
        """Called when the stroke reaches the top corner — pause and show the
        Hope toggle so the player can decide before the outcome is locked."""
        if self._enc.phase != WoundEncPhase.AWAITING_SAVE:
            return
        self._awaiting_hope_choice = True
        outcome = self._pending_outcome
        self._outcome_lbl.text       = self._wound_outcome_label(outcome)
        self._outcome_lbl.text_color = T.k(self._wound_outcome_color(outcome))
    ...
    def _finalize_wound_save_choice(self, outcome: str, after_choice):
        """Lock the outcome colour, resume the stroke sweep, then call after_choice."""
        stab = self._enc_stage_tabs.get("save")
        if stab is None:
            return
        self._awaiting_hope_choice = False
        self._hope_row.height  = 0
        ...
        Animation(
            text_color=T.k(stab._cap_hex), duration=0.20, t="out_quad"
        ).start(self._enc_roll_num)
        self._outcome_lbl.text       = self._wound_outcome_label(outcome)
        self._outcome_lbl.text_color = T.k(stab._cap_hex)
        self._outcome_lbl.opacity    = 0
        Animation(opacity=1, duration=0.22, t="out_cubic").start(self._outcome_lbl)
        Clock.schedule_once(lambda dt: self._sync_shell_height(), 0.02)
        stab.resume_commit_anim(
            on_complete=lambda: Clock.schedule_once(after_choice, 0.10)
        )

    def _use_hope(self):
        """Consume the Hope resource and upgrade the wound outcome to pass5."""
        ...
        app.refresh_all()
        # Upgrade to pass5 — no wound, no sanity loss
        self._pending_outcome      = "pass5"
        self._pending_wound_desc   = ""
        self._pending_wound_effect = ""
        self._pending_wound_sev    = ""
        self._pending_wound_roll   = 0
        self._pending_sanity_loss  = 0
        self._pending_san_rolls    = []
        self._finalize_wound_save_choice(
            "pass5",
            lambda _dt: Clock.schedule_once(lambda dt: self._do_auto_complete(), 0.60),
        )

    def _decline_hope(self):
        """Keep the original failed outcome; proceed to the sanity/wound stages."""
        if not self._awaiting_hope_choice:
            return
        self._finalize_wound_save_choice(
    ...
    def _on_wound_encounter(self, *_):
        ...
        # Hope triggers on any fail (fail / fail5) if the resource is available
        has_hope_choice = (outcome in ("fail", "fail5")) and self._app().state.hope
        ...
        def _on_ready():
                    ...
                    def _open_save(dt):
                        ...
                        def _begin_wrap(dt_bw):
                            ...
                            self._play_tab_commit(
                                ...
                                on_top_reached=(
                                    self._show_hope_choice if has_hope_choice else None
                                ),
                            )
                        ...
                        def _start_save_spin(dt2):
                            def _save_land(dt3):
                                ...
                                # When hope is available the stroke is already paused
                                # at the top corner — cap_hex and outcome display are
                                # deferred to _finalize_wound_save_choice instead.
                                stab = self._enc_stage_tabs["save"]
                                stab._commit_cb = None  # stage advances via Clock below

                                if has_hope_choice:
                                    # Hope toggle was shown by on_top_reached; leave
                                    # cap_hex and outcome label for the decision handler.
                                    return
            ...
            # Store sanity-start callback so _decline_hope can resume the flow
            # after the user decides not to use Hope on a failed save.
            if has_hope_choice:
                self._pending_after_save = _start_sanity
