...
from ui_utils import (
    ...
    populate_rules_section, FillSwipeTitle, MorphArrow,
)
import theme as T
...
#   Level 1 — desc open  : wound effect text + "ENCOUNTER DETAILS" button + MorphArrow.
...
class EncounterWoundCard(MDBoxLayout):
    ...
    def __init__(self, title: str, subtitle: str, effect: str,
        ...
        # Button row: flat button + morph arrow
        ...
        self._btn_row.add_widget(self._arrow)
    ...
    def on_touch_down(self, touch):
        ...
        # Let interactive children (button, tabs) handle first.
    ...
    def _desc_inner_h(self) -> float:
        ...
        return p[1] + body + dp(6) + dp(36) + p[3]
...
class WoundsTab(MDBoxLayout):
    ...
    def _on_wound_encounter(self, *_):
        ...
        def _on_ready():
            ...
            def _start_dc_spin(dt):
                ...
                def _after_dc_commit():
                    ...
                    def _open_save(dt):
                        ...
                        # ── Pre-stroke: top stroke sweeps while the spin runs ──
                        # appear() animation = 0.32 s.  Spin lands at
                        # _TAB_SETTLE_DELAY + _spin_t + 0.30 after _open_save.
                        # Pre-stroke runs from tab-extend to spin-land = 1.489 s.
                        _pre_dur = (self._TAB_SETTLE_DELAY + _spin_t + 0.30) - 0.32
                        def _begin_pre_stroke(dt_ps):
                            stab = self._enc_stage_tabs["save"]
                            top_w = max(dp(1), stab.width - _EncTab._RAIL_W)
                            stab._stroke_col.rgba = (*T.k(stab._main_hex)[:3], 1.0)
                            stab._stroke_top_rect.size = (0, _EncTab._SIDE_W)
                            stab._stroke_top_rect.pos = (
                                stab.x + _EncTab._RAIL_W,
                                stab.top - _EncTab._SIDE_W)
                            _ps_start = Clock.get_boottime()
                            def _ps_tick(dt_t,
                                         _stab=stab, _tw=top_w, _dur=_pre_dur,
                                         _t0=_ps_start):
                                prog = min(1.0,
                                           (Clock.get_boottime() - _t0) / _dur)
                                _stab._stroke_top_rect.pos = (
                                    _stab.x + _EncTab._RAIL_W,
                                    _stab.top - _EncTab._SIDE_W)
                                _stab._stroke_top_rect.size = (
                                    _tw * prog, _EncTab._SIDE_W)
                                return prog < 1.0
                            stab._pre_stroke_evt = Clock.schedule_interval(
                                _ps_tick, 1 / 60)
                        Clock.schedule_once(_begin_pre_stroke, 0.32)
                        ...
                        def _start_save_spin(dt2):
                            def _save_land(dt3):
                                ...
                                # Cancel the pre-stroke and hand off to play_commit_anim.
                                # Because Canvas instructions batch within the same frame,
                                # restoring _stroke_top_rect AFTER play_commit_anim clears
                                # it means the GPU never sees the cleared state — no flicker.
                                stab = self._enc_stage_tabs["save"]
                                if getattr(stab, "_pre_stroke_evt", None):
                                    stab._pre_stroke_evt.cancel()
                                    stab._pre_stroke_evt = None
                                stab._cap_hex = self._wound_outcome_color(outcome)
                                self._play_tab_commit(
                                    "save", total_duration=0.84,
                                    on_complete=_after_save_commit)
                                # play_commit_anim reset _commit_prog=0 and cleared
                                # stroke art.  Pre-advance past Phase 1 so side-tab
                                # colour begins immediately (top stroke already drawn).
                                top_w      = max(dp(1), stab.width - _EncTab._RAIL_W)
                                total_path = top_w + stab.height + top_w
                                trace_end  = 0.84 * 0.65
                                stab._commit_prog = trace_end * (top_w / total_path)
                                stab._stroke_top_rect.pos = (
                                    stab.x + _EncTab._RAIL_W,
                                    stab.top - _EncTab._SIDE_W)
                                stab._stroke_top_rect.size = (top_w, _EncTab._SIDE_W)
                                ...
                                Clock.schedule_once(_show_outcome, 0.30)
            ...
            # ── Stage 4: WOUND ────────────────────────────────────────────────
            # The wound panel has no spin animation, so use a short sweep duration
            # that matches the fear encounter's choice tab (default 0.84 s).
            _wnd_sweep = 0.84
                ...
                def _after_reveal(dt2):
                    self._play_tab_commit(
                        ...
                        on_complete=lambda: Clock.schedule_once(
                            lambda dt3: self._do_auto_complete(), 0.20))

                Clock.schedule_once(
                    _after_reveal,
                    self._TAB_SETTLE_DELAY + self._STAGE_BOX_EXPAND_DELAY
                    + self._STAGE_CARD_REVEAL_DURATION + 0.50)
