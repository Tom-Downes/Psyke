...
class HeaderCard(BoxLayout):
    ...
    def _apply_responsive_layout(self, *_):
        ...
        reset_w = max(round(dp(48)), round(dp(68) * scale))
...
class _ResetBtn(FloatLayout):
    ...
    def __init__(self, on_tap, **kwargs):
        ...
        with self.canvas.before:
            ...
            self._base_bg_col = Color(*T.k(T.BG_CARD))
        ...
        self._x_lbl.bind(size=lambda inst, *_: setattr(inst, "text_size", inst.size))

        self.add_widget(self._x_lbl)
    ...
    def set_scale(self, scale: float):
        ...
        self._slot_w = max(dp(10), dp(13) * self._scale)
    ...
    def _redraw(self, *_):
        ...
        # X: starts small in the right slot, grows and travels to centre while holding
        x_base_w = dp(12) * self._scale
        x_size_w = x_base_w + dp(38) * self._scale * self.slide_t
        x_font_sp = max(10.0, (11 + 18 * self.slide_t) * self._scale)
        ...
        self._x_lbl.font_size = f"{x_font_sp:.0f}sp"
        self._x_lbl.size = (x_size_w, self.height)
        self._x_lbl.pos = (x_lbl_x, self.y)
        self._x_lbl.opacity = 1.0
